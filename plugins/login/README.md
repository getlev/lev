...
The **login plugin** for [Lev](http://github.com/getgrav/grav) adds login, basic ACL, and session wide messages to Lev.  It is designed to provide a way to secure front-end and admin content throughout Lev.
...
Added new `$lev['login']->login()` and `$lev['login']->logout()` functions for you to use.
...
|| [ -p, --password=PASSWORD ]       | The password. Ensure the password respects Lev's password policy. **Note that this option is not recommended because the password will be visible by users listing the processes.** |
...
|| [ -u, --user=USER ]               | The username.                                                   |
|| [ -p, --password=PASSWORD ]       | The new password. Ensure the password respects Lev's password policy. **Note that this option is not recommended because the password will be visible by users listing the processes.** |


### CLI Example
```
> bin/plugin login new-user -u joeuser -p 8c9sRCeBExAiwk -e joeuser@lev.org -P b -N "Joe User" -t "Site Administrator"
Creating new user
...
Enter an email:   joeuser@lev.org
...
rememberme:
  ...
  name: lev-rememberme                     # Name prefix of the session cookie
...
By default Lev will redirect to the prior page visited before entering the login process.  Any page is fair game unless you manually set:
...
```
{% if config.plugins.login.enabled and lev.user.username %}
...
Here are two ways you can do it, but of course Lev is flexible and you can come up with other ways too.
...
Once the user is registered, Lev redirects the user to the `display` page with the `message` message.

The only field strictly required by Lev is `username`. Then the other fields can be added as needed.
...
Lev allows disabled user accounts, so we can take advantage of this functionality and add a new user, but with a disabled state. Then we can send an email to the user, asking to validate the email address.
