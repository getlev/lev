...
class ChangePasswordCommand extends ConsoleCommand
{
    ...
    protected function configure()
    {
        $this
            ...
            ->addOption(
                ...
                'p',
                InputOption::VALUE_REQUIRED,
                "The password. Note that this option is not recommended because the password will be visible by users listing the processes. You should also make sure the password respects Lev's password policy."
            )
    ...
    protected function serve()
    {
        ...
        $lev = Lev::instance();
        if (!isset($lev['login'])) {
            $lev['login'] = new Login($lev);
        }
        $this->login = $lev['login'];
