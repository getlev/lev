...
namespace Lev\Plugin\Login\Events;
 ...
 * @package Lev\Plugin\Login\Events
 *
 * @property int                $status
...
class UserLoginEvent extends Event
{
    ...
    public function __construct(array $items = [])
    {
        ...
        if (!$this->offsetExists('session') && isset(Lev::instance()['session'])) {
            $this->offsetSet('session', Lev::instance()['session']);
        }
        if (!$this->offsetExists('user')) {
            /** @var UserCollectionInterface $users */
            $users = Lev::instance()['accounts'];
