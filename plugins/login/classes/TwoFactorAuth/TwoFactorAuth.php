...
namespace Lev\Plugin\Login\TwoFactorAuth;

use Lev\Common\Lev;
use Lev\Common\HTTP\Client;
...
/**
 * Class TwoFactorAuth
 * @package Lev\Plugin\Login\RememberMe
 */
class TwoFactorAuth
{
    protected $twoFa;
    ...
    {
        $this->twoFa = new Auth('Lev', 6, 30, 'sha1', new BaconQrProvider);
    }
    ...
    {
        $label = $username . ':' . Lev::instance()['config']->get('site.title');
        $secret = str_replace(' ', '', $secret);
