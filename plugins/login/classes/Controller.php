...
class Controller
{
     ...
     * @param array  $post
     */
    public function __construct(Lev $lev, $action, $post = null)
    {
        $this->lev = $lev;
        $this->action = $action;
    ...
    public function execute()
    {
        ...
        try {
            ...
            $this->lev['log']->error('plugin.login: '. $e->getMessage());
        }
    ...
    public function taskLogin()
    {
        ...
        // Is twofa enabled?
        $twofa = $this->lev['config']->get('plugins.login.twofa_enabled', false);
        ...
        if ($user->authenticated) {
            if ($user->authorized) {
                ...
                $event->defRedirect(
                    $this->lev['session']->redirect_after_login ?:
                        $login_redirect ?: $this->lev['uri']->referrer('/', '', true)
                );
            } else {
                $redirect_to_login = $this->lev['config']->get('plugins.login.redirect_to_login');
                $redirect_route = $redirect_to_login ? $this->login->getRoute('login') : null;
                $event->defRedirect($redirect_route ?? $this->lev['uri']->referrer('/', '', true));
            }
        } else {
            if ($user->authorized) {
    ...
    public function taskTwoFa()
    {
        ...
        if (!$code || !$secret || !$twoFa->verifyCode($secret, $code)) {
            ...
            $this->lev->fireEvent('onUserLoginFailure', $event);
            ...
            if (!$event->getRedirect()) {
                $redirect_to_login = $this->lev['config']->get('plugins.login.route_to_login');
            ...
            if (!$event->getRedirect()) {
                ...
                $event->setRedirect(
                    $this->lev['session']->redirect_after_login ?: $login_redirect ?: $this->lev['uri']->referrer('/', '', true),
                    303
                );
            }
        }
    ...
    public function taskLogout()
    {
        ...
        $this->lev['session']->setFlashCookieObject(LoginPlugin::TMP_COOKIE_NAME, ['message' => $this->lev['language']->translate('PLUGIN_LOGIN.LOGGED_OUT'),
            'status' => 'info']);
            ...
            if ($user && !empty($user->reset) && $user->exists()) {
                ...
                if ($good_token === $token) {
                    if (time() > $expire) {
                        ...
                        $this->lev->redirectLangSafe($this->login->getRoute('forgot') ?? '/');
    ...
    {
        if ($this->redirect) {
            $this->lev->redirectLangSafe($this->redirect, $this->redirectCode);
        }
    }
     ...
     * @deprecated 2.5.0 Use $lev['login']->rememberMe() instead
     */
