...
1. [](#new)
    * Require **Lev 1.7.32**, **Form 6.0.0** and **Email 3.1.6**
    * Added support for fully customizable email templates
    * Added `Lev\PluginsLogin\Email` class to simplify sending emails
...
1. [](#bugfix)
    * Fixed double language in redirection after successful login [lev#3411](https://github.com/getgrav/grav/issues/3411)
...
1. [](#new)
   * Require **Lev 1.7.19**, **Form 5.1.0** and **Email 3.1.0**
   * Added support for adding login pages by `$lev['login']->addPage()`
   * Added support for getting all login routes by `$lev['login']->getRoute()`
...
1. [](#bugfix)
    * Fixed failing Flex User validation if user has changed in the filesystem (requires Lev 1.7.13) [#278](https://github.com/getgrav/grav-plugin-login/issues/278)
...
1. [](#new)
    * Lev 1.7 only: Added `Sync User in Session` feature [#252](https://github.com/getgrav/grav-plugin-login/issues/252)
1. [](#improved)
...
1. [](#new)
    ...
    * Change session ID during login to prevent session fixation (requires Lev 1.7)
1. [](#bugfix)
...
1. [](#improved)
    ...
    * Use `$lev['users']` collection instead of deprecated static calls
...
1. [](#new)
    * Store remember me triplets into `app://data/rememberme` instead of storing them into the cache
...
1. [](#new)
    ...
    * Updated plugin dependencies (Lev >= 1.4.5, Form >=2.13.4, Email >=2.7.0)
1. [](#improved)
...
1. [](#bugfix)
    * Don't allow Profile saving if a Lev user account doesn't exist (OAuth/LDAP users for example)
...
1. [](#new)
    * Added `$lev['login']->login()` and `$lev['login']->logout()` functions with event hooks
    * Added `$lev['login']->getRateLimiter($context)` function
...
1. [](#bugfix)
    ...
    * Improved validation of email to support RFC5322 [Lev#1648](https://github.com/getgrav/grav/issues/1648)
...
1. [](#bugfix)
    * Lev plugin cli error on password change [#120](https://github.com/getgrav/grav-plugin-login/issues/120)
...
2. [](#improved)
    * Added blueprints for Lev Admin plugin (multi-language support!)
