...
The **email plugin** for [Lev](http://github.com/getgrav/grav) adds the ability to send email. This is particularly useful for the **admin** and **login** plugins.
...
In order to get ZOHO working with Lev, you need to send email via a user account.  You can either use your users' password or generate an **App Password** via your ZOHO account (clicking on your avatar once logged in), then navigating to `Two Factor Authentication -> App Passwords -> Generate`. Just enter a unique app name (i.e. `Lev Website`).
...
For performance reasons, it's often desirable to queue emails and send them in batches, rather than forcing Lev to wait while an email is sent.  This is because email servers are sometimes slow and you might not want to wait for the whole email-sending process before continuing with Lev processing.

To address this, you can enable the **Email Queue** and this will ensure all email's in Lev are actually sent to the queue, and not sent directly.  In order for the emails to be actually sent, you need to flush the queue.  By default this is handled by the **Lev Scheduler**, so you need to ensure you have that enabled and setup correctly or **your emails will not be sent!!!**.
...
```php
        ...
        $message = $this->lev['Email']->message($subject, $content, 'text/html')
            ->setFrom($from)
            ->setTo($to);

        $sent = $this->lev['Email']->send($message);
```
...
You can specify a Twig template for HTML rendering, else Lev will use the default one `email/base.html.twig` which is included in this plugin.  You can also specify a custom template that extends the base, where you can customize the `{% block content %}` and `{% block footer %}`.  For example:
