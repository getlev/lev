...
1. [](#bugfix)
    ...
    * Fixed `LevForm.config` JS to have correct `current_url` and `current_params` settings
...
1. [](#improved)
    * Improve Lev 1.7 support by not using deprecated `$page->modular()` call
...
1. [](#new)
    ...
    * Add ability to Sanitize SVGs on upload (Lev 1.7+ required)
1. [](#improved)
...
1. [](#bugfix)
    ...
    * Fixed issue with Form JS when pipeline is enabled [lev#2592](https://github.com/getgrav/grav/issues/2592)
...
1. [](#bugfix)
    * Fixed file upload when `$lev['user']` is not set [#352](https://github.com/getgrav/grav-plugin-form/issues/352)
...
1. [](#new)
    ...
    * Form no longer extends `Lev\Common\Iterator` (may have some backward compatibility issues with plugins, likely not)
    ...
    * Added `$lev['forms']` to allow plugins to better use forms [#307](https://github.com/getgrav/grav-plugin-form/pull/307)
    ...
    * Requires Lev 1.6.0-beta.7 (and optionally Admin 1.9.0-beta.7)
...
1. [](#improved)
    ...
    * Now using the new core Lev language prefix
...
1. [](#bugfix)
    * Fixed regression with select field, causing issues with filepicker field [lev-plugin-admin#1441](https://github.com/getgrav/grav-plugin-admin/issues/1441)
...
1. [](#bugfix)
    * Fixed regression in v2.11.4: Call to a member function post() on null [lev#1720](https://github.com/getgrav/grav/issues/1720)
...
1. [](#improved)
    ...
    * Added the ability for a form to have an `http_response_code` and use it for `form-messages.html.twig` (requires Lev v1.3.6+)
...
1. [](#new)
    * Bundled as RC release for Lev/Admin RC releases
...
1. [](#bugfix)
    * Added check for valid Lev forms before trying to create a form object
...
1. [](#bugfix)
    ...
    * Fixed errant reference to Lev DI container in Form#getPagePathFromToken [#105](https://github.com/getgrav/grav-plugin-form/issues/105)
...
1. [](#improved)
    * Added support for Lev's autoescape twig setting
