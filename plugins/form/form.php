...
use Exception;
use Lev\Common\Data\ValidationException;
...
use Lev\Plugin\Form\TwigExtension;
...
class FormPlugin extends Plugin
{
    ...
    {
        return version_compare(LEV_GVERSION, '1.7', '>');
    }
    ...
    public function onPluginsInitialized(): void
    {
        ...
        class_alias(Form::class, 'Lev\Plugin\Form');

        $this->lev['forms'] = function () {
            $forms = new Forms();
            $event = new Event(['forms' => $forms]);

            $this->lev->fireEvent('onFormRegisterTypes', $event);
    ...
    public function onPageInitialized(): void
    {
        ...
        if ($this->shouldProcessForm()) {
            ...
            if ($form instanceof Form) {
                ...
                if (empty($this->json_response)) {
                    if ($task === 'clear-state') {
                        ...
                        $this->lev->redirect($redirect, 303);
                    } else {
                        $form->post();
                        $submitted = true;
                    }
            ...
            if (!$submitted && null === $uri->extension()) {
                ...
                if ($flash = $this->lev['session']->getFlashObject('files-upload')) {
            ...
            foreach ($pageForms as $name => $form) {
                ...
                if ($form->get('remember_redirect')) {
                    ...
                    $uniqueId = $route->getLevParam($formParam);
                    ...
                    $route = $route->withLevParam($formParam, $form->getUniqueId());
    ...
    {
        $this->lev['twig']->twig()->addFunction(
            new TwigFunction('forms', [$this, 'getForm'])
        );

        if (Environment::VERSION_ID > 20000) {
            // Twig 2/3
            $this->lev['twig']->twig()->getExtension(EscaperExtension::class)->setEscaper(
                'yaml',
            ...
            // Twig 1.x
            $this->lev['twig']->twig()->getExtension(CoreExtension::class)->setEscaper(
                'yaml',
                function ($twig, $string, $charset) {
                    return Yaml::dump($string);
                }
    ...
    {
        $this->lev['twig']->twig->addExtension(new TwigExtension());
    }
    ...
    public function onTwigVariables(Event $event = null): void
    {
        ...
        if ($this->config->get('plugins.form.built_in_css')) {
            $this->lev['assets']->addCss('plugins://form/assets/form-styles.css');
        }
        ...
        switch ($action) {
            case 'captcha':
                ...
                if (!$resp->isSuccess()) {
                    ...
                    $message = $this->lev['language']->translate('PLUGIN_FORM.ERROR_VALIDATING_CAPTCHA');
                    ...
                    $this->lev['log']->addWarning('Form reCAPTCHA Errors: [' . $uri->route() . '] ' . json_encode($errors));
            ...
            case 'ip':
                ...
                break;
            case 'message':
                $translated_string = $this->lev['language']->translate($params);
                $vars = array(
                    'form' => $form
                );
                ...
                break;
            case 'redirect':
                $this->lev['session']->setFlashObject('form', $form);
                $url = ((string)$params);
                $vars = array(
                    'form' => $form
                );
                ...
                if ($message) {
                    $this->lev['messages']->add($form->message, 'success');
                }
    ...
    protected function shouldProcessForm(): bool
    {
        ...
        if ($status && $form = $this->form()) {
            ...
            if (isset($form->template)) {
                $this->lev['page']->template($form->template);
            }
            ...
            if ($refresh_prevention && $unique_form_id) {
                if ($this->lev['session']->unique_form_id !== $unique_form_id) {
                    $isJson = $uri->extension() === 'json';
                    // AJAX tasks aren't submitting
                    if (!$isJson || !($uri->post('__form-file-uploader__') || $uri->post('__form-file-remover__'))) {
                        $this->lev['session']->unique_form_id = $unique_form_id;
                    }
                } else {
                    $status = false;
                    $form->message = $this->lev['language']->translate('PLUGIN_FORM.FORM_ALREADY_SUBMITTED');
                    $form->status = 'error';
                }
            }
        }
    ...
    protected function form(PageInterface $page = null)
    {
        ...
        if (null === $form) {
            ...
            $form_name = $this->lev['uri']->post('__form-name__', FILTER_SANITIZE_STRING) ?? '';
            $unique_id = $this->lev['uri']->post('__unique_form_id__', FILTER_SANITIZE_STRING) ?? '';
