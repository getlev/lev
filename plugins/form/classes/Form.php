...
use Lev\Framework\Form\FormFlashFile;
...
class Form implements FormInterface, ArrayAccess
{
    ...
    public function initialize()
    {
        ...
        // Fire event
        $lev = Lev::instance();
        $lev->fireEvent('onFormInitialized', new Event(['form' => $this]));
    ...
    public function uploadFiles()
    {
        ...
        $lev->fireEvent('onFormUploadSettings', new Event(['settings' => &$settings, 'post' => $post]));
        ...
        if ($settings->filesize > 0 && $upload['file']['size'] > $settings->filesize) {
            ...
            return [
                'status'  => 'error',
                'message' => $language->translate('PLUGIN_FORM.EXCEEDED_LEV_FILESIZE_LIMIT')
            ];
        }
        ...
        if (method_exists('Lev\Common\Security', 'sanitizeSVG') && Utils::contains($mime, 'svg', false)) {
        ...
        $flash = $this->getFlash();
        $flash->setUrl($url)->setUser($lev['user'] ?? null);
    ...
    public function post()
    {
        ...
        if ($post) {
            ...
            if (!$this->values->get('form-nonce') || !Utils::verifyNonce($this->values->get('form-nonce'), 'form')) {
                ...
                $event = new Event(['form' => $this,
                    'message' => $lev['language']->translate('PLUGIN_FORM.NONCE_NOT_VALIDATED')
                ]);
                $lev->fireEvent('onFormValidationError', $event);
        ...
        try {
            $lev->fireEvent('onFormPrepareValidation', new Event(['form' => $this]));

            $this->data->validate();
            $this->data->filter();

            $lev->fireEvent('onFormValidationProcessed', new Event(['form' => $this]));
        ...
        if (is_array($process)) {
            foreach ($process as $action => $data) {
                ...
                $lev->fireEvent('onFormProcessed', $event);
        ...
        if ($redirect) {
            $lev->redirect($redirect, $redirect_code);
        }
    }
        ...
        foreach ($fields as $key => $uploads) {
            ...
            foreach ($uploads as $upload) {
                ...
                if (!is_dir($folder) && !@mkdir($folder, 0777, true) && !is_dir($folder)) {
                    $lev = Lev::instance();
                    throw new RuntimeException(sprintf($lev['language']->translate('PLUGIN_FORM.FILEUPLOAD_UNABLE_TO_MOVE', null, true), '"' . $upload->getClientFilename() . '"', $destination));
                }
    ...
    public function legacyUploads()
    {
        ...
        $lev->fireEvent('onFormStoreUploads', new Event(['form' => $this, 'queue' => &$queue, 'post' => $post]));
            ...
            if (is_array($queue)) {
                foreach ($queue as $key => $files) {
                    foreach ($files as $destination => $file) {
                        ...
                        if (!is_dir($folder) && !@mkdir($folder, 0777, true) && !is_dir($folder)) {
                            $lev = Lev::instance();
                            throw new RuntimeException(sprintf($lev['language']->translate('PLUGIN_FORM.FILEUPLOAD_UNABLE_TO_MOVE', null, true), '"' . $file['tmp_name'] . '"', $destination));
                        }
    ...
    {
        $route = Uri::getCurrentRoute()->withExtension('json')->withLevParam('task', 'file-upload');
    ...
    {
        $route = Uri::getCurrentRoute()->withExtension('json')->withLevParam('task', 'file-remove');
    ...
    {
        $types = Lev::instance()['plugins']->formFieldTypes;
