...
export default class PageMedia extends FilesField {
    constructor({ container = '#lev-dropzone', options = {} } = {}) {
    ...
    updateThumbsSize() {
      const status = JSON.parse(Cookies.get('lev-admin-pagemedia') || '{}');
...
export const updateMediaSizes = (input, width, store = true) => {
  const storageLocation = input.dataset.storageLocation || 'lev-admin-pagemedia';
...
export const updateMediaCollapseStatus = (element, store = true) => {
  const storageLocation = element.dataset.storageLocation || 'lev-admin-pagemedia';
  const status = JSON.parse(Cookies.get(storageLocation) || '{}');
