...
export default class ArrayField {
    ...
    addArray(list) {
        ...
        list.find('[data-lev-array-type="container"]').each((index, container) => {
            ...
            container.data('array-sort', new Sortable(container.get(0), {
                ...
                animation: 150,
                onUpdate: () => {
                    const item = container.find('[data-lev-array-type="row"]:first');
    ...
    refreshNames(template) {
        ...
        inputs.each((index, input) => {
            input = $(input);
            const preserved_name = input.closest('[data-lev-array-name]');
            const name = `${preserved_name.attr('data-lev-array-name')}[${index}]`;
