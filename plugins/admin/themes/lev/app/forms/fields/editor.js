...
export default class EditorField {
    constructor(options) {
        ...
        $('[data-lev-editor]').each((index, editor) => this.addEditor(editor));

        $(() => { body.trigger('lev-editor-ready'); });
        body.on('mutation._lev', this._onAddedNodes.bind(this));

        body.on('mouseup._lev', () => {
            if (!IS_MOUSEDOWN) { return true; }
            body.unbind('mousemove._lev');
            IS_MOUSEDOWN = false;
        });
        body.on('mousedown._lev', '.lev-editor-resizer', (event) => {
            event && event.preventDefault();
            IS_MOUSEDOWN = true;

            let target = $(event.currentTarget);
            let container = target.siblings('.lev-editor-content');
            let editor = container.find('.CodeMirror');
            let codemirror = container.find('textarea').data('codemirror');

            body.on('mousemove._lev', (event) => {
    ...
    addEditor(textarea) {
        ...
        let options = Object.assign(
            ...
            textarea.data('lev-editor').codemirror
        );
        ...
        if (options.toolbar === false) {
            textarea.data('toolbar').ui.navigation.addClass('lev-editor-hide-toolbar');
        }
    ...
    _onAddedNodes(event, target/* , record, instance */) {
        let editors = $(target).find('[data-lev-editor]');
...
export class Toolbar {
    static templates() {
        return {
            navigation: `
                <div class="lev-editor-toolbar">
                    <div class="lev-editor-actions"></div>
                    <div class="lev-editor-modes"></div>
                </div>
            `
        };
    }
    ...
    constructor(editor) {
        ...
        this.editor.parent('.lev-editor-content')
    ...
    renderButtons() {
        ...
        ['actions', 'modes'].forEach((type) => {
            this.ui.navigation.find(`.lev-editor-${type}`).empty().append('<ul />');
    ...
    renderButton(button, type, location = null) {
        Object.keys(button).forEach((key) => {
            ...
            if (!~this.codemirror.options.ignore.indexOf(key) && (!obj.modes.length || obj.modes.indexOf(this.codemirror.options.mode) > -1)) {
                ...
                let element = $(`<li class="lev-editor-button-${key}"><a class="hint--top" ${hint}>${obj.label}</a></li>`);
                (location || this.ui.navigation.find(`.lev-editor-${type} ul:not(.dropdown-menu)`)).append(element);
