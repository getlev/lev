    ...
    states: [{
        code: {
            ...
            action({ codemirror, button, textarea, ui }) {
                if (textarea.data('lev-editor-mode') === 'editor') {
                ...
                button.on('click.states.editor', () => {
                    ...
                    button.addClass('editor-active');
                    textarea.data('lev-editor-mode', 'editor');
                    let previewContainer = textarea.data('lev-editor-preview-container');
                    let content = textarea.parent('.lev-editor-content');

                    content.addClass('is-active');
                    ui.navigation.find('.lev-editor-actions').css('visibility', 'visible');
                    if (previewContainer) {
        ...
        preview: {
            ...
            modes: ['gfm', 'markdown'],
            action({ codemirror, button, textarea, ui }) {
                if (textarea.data('lev-editor-mode') === 'preview') {
                    button.addClass('editor-active');
                }
                button.on('click.states.preview', () => {
                    ...
                    button.siblings().removeClass('editor-active');
                    button.addClass('editor-active');
                    textarea.data('lev-editor-mode', 'preview');

                    if (!previewContainer) {
                        previewContainer = $('<div class="lev-editor-preview" />');
                        content.after(previewContainer);
                        textarea.data('lev-editor-preview-container', previewContainer);
                    }

                    previewContainer.css({ height: content.height() });
                    previewContainer.addClass('is-active');
                    content.removeClass('is-active');
                    ui.navigation.find('.lev-editor-actions').css('visibility', 'hidden');

                    let url = `${textarea.data('lev-urlpreview')}/task${config.param_sep}processmarkdown`;
        ...
        fullscreen: {
            ...
            action({ codemirror, button, textarea }) {
                button.on('click.editor.fullscreen', () => {
                    let container = textarea.closest('.lev-editor');
                    let wrapper = codemirror.getWrapperElement();
                    let contentWrapper = $('.content-wrapper');

                    if (!container.hasClass('lev-editor-fullscreen')) {
                        ...
                        wrapper.style.width = '';
                        wrapper.style.height = textarea.parent('.lev-editor-content').height() + 'px';
                        global.document.documentElement.style.overflow = 'hidden';

                        let hints = container.find('.lev-editor-toolbar .hint--top');
                        ...
                        let hints = container.find('.lev-editor-toolbar .hint--bottom');
                    ...
                    container.toggleClass('lev-editor-fullscreen');
