...
export default class CollectionsField {
  ...
  addItem(event) {
    ...
    $('[data-lev-field="toggleable"] input[type="checkbox"]').trigger('change');
  }
  ...
  reindex(list, items) {
    ...
    items.each((index, item) => {
      ...
      ['name', 'data-lev-field-name', 'for', 'id', 'data-lev-file-settings', 'data-file-post-add', 'data-file-post-remove', 'data-lev-array-name', 'data-lev-elements'].forEach((prop) => {
        item.find('[' + prop + '], [_' + prop + ']').each(function() {
          ...
          if (prop === 'name' && element.data('levArrayType')) {
            const match_index = element.attr(prop).match(/\[[0-9]{1,}\]$/);
            const pattern = element[0].closest('[data-lev-array-name]').dataset.levArrayName;
