import $ from 'jquery';
import { config, uri_params } from 'lev-config';
...
export default class FilePickerField {
    ...
    constructor(options) {
        ...
        $('[data-lev-filepicker]').each((index, element) => this.addItem(element));
        $('body').on('mutation._lev', this._onAddedNodes.bind(this));
    }

    _onAddedNodes(event, target/* , record, instance */) {
        let fields = $(target).find('[data-lev-filepicker]');
    ...
    addItem(element) {
        ...
        let getData = function getData(field, callback, mode = 'all') {
            ...
            let parent = field.closest('[data-lev-filepicker]');
