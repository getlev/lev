...
class Popularity
{
    ...
    {
        $this->config = Lev::instance()['config'];

        $this->data_path     = Lev::instance()['locator']->findResource('log://popularity', true, true);
    ...
    public function trackHit()
    {
        ...
        if (!Lev::instance()['browser']->isHuman()) {
            return;
        }

        // Respect visitors "do not track" setting
        if (!Lev::instance()['browser']->isTrackable()) {
            return;
        }

        /** @var PageInterface $page */
        $page         = Lev::instance()['page'];
        $relative_url = str_replace(Lev::instance()['base_url_relative'], '', $page->url());
        ...
        $this->updateVisitors(Lev::instance()['uri']->ip());
