...
use Lev\Framework\RequestHandler\Exception\RequestException;
...
class AdminController extends AdminBaseController
{
     ...
     * @return void
     */
    public function initialize(Lev $lev = null, $view = null, $task = null, $route = null, $post = null)
    {
        $this->lev = $lev;
        $this->admin = $this->lev['admin'];
        $this->view = $view;
        ...
        $this->lev->fireEvent('onAdminControllerInit', new Event(['controller' => &$this]));
    }
    ...
    protected function taskClearCache()
    {
        ...
        $clear_type = $this->lev['uri']->param('cleartype');
    ...
    public function taskSave()
    {
        ...
        $this->lev['twig']->twig_vars['current_form_data'] = (array)$this->data;
        ...
        switch ($this->view) {
            ...
            default:
                if ($this->saveDefault()) {
                    $route = $this->lev['uri']::getCurrentRoute();
                    $this->setRedirect($route->withLevParam('task', null)->toString(), 302);
                    $this->redirect();
                }
    ...
    protected function saveDefault()
    {
        ...
        if ($obj) {
            ...
            $this->lev->fireEvent('onAdminSave', new Event(['object' => &$obj]));
            $obj->save();
            $this->admin->setMessage($this->admin::translate('PLUGIN_ADMIN.SUCCESSFULLY_SAVED'), 'info');
            $this->lev->fireEvent('onAdminAfterSave', new Event(['object' => $obj]));
        }
    ...
    public function taskRegenerate2FASecret()
    {
        ...
        try {
            ...
            $twoFa = $this->lev['login']->twoFactorAuth();
            ...
            if ($user instanceof \Lev\Common\User\DataUser\User) {
    ...
    protected function saveUser()
    {
        ...
        if (!$this->admin->authorize(['admin.super', 'admin.users'])) {
            ...
            if ($user->username !== $this->lev['user']->username) {
        ...
        if ($user) {
            // Event to manipulate data before saving the object
            $this->lev->fireEvent('onAdminSave', new Event(['object' => &$user]));
            $user->save();
            $this->admin->setMessage($this->admin::translate('PLUGIN_ADMIN.SUCCESSFULLY_SAVED'), 'info');
            $this->lev->fireEvent('onAdminAfterSave', new Event(['object' => $user]));
        }
        ...
        if ($user->username === $this->lev['user']->username) {
            ...
            $this->lev['user']->undef('avatar');
            $this->lev['user']->merge($users->load($this->admin->route)->toArray());
        }
        ...
        try {
            ...
            foreach ($notifications as $type => $type_notifications) {
                if ($filter_types && in_array($type, $filter_types, true)) {
                    ...
                    $notification_data[$type] = $this->lev['twig']->processTemplate($twig_template, ['notifications' => $type_notifications]);
                }
            }
    ...
    protected function taskHideNotification()
    {
        ...
        $notification_id = $this->lev['uri']->param('notification_id');
        ...
        $filename = $this->lev['locator']->findResource('app://data/notifications/' . $this->lev['user']->username . YAML_EXT, true, true);
    ...
    protected function taskGetNewsFeed()
    {
        ...
        try {
            ...
            $feed_data = $this->lev['twig']->processTemplate('partials/feed-block.html.twig', ['feed' => $feed]);
    ...
    protected function taskBackup()
    {
        ...
        $param_sep = $this->lev['config']->get('system.param_sep', ':');
        $download = $this->lev['uri']->param('download');

        try {
            if ($download) {
                $filename = Utils::basename(base64_decode(urldecode($download)));
                $file = $this->lev['locator']->findResource("backup://{$filename}", true);
        ...
        $url = rtrim($this->lev['uri']->rootUrl(false), '/') . '/' . trim($this->admin->base,
    ...
    protected function taskBackupDelete()
    {
        ...
        $backup = $this->lev['uri']->param('backup', null);
    ...
    public function taskEnable()
    {
        ...
        if ($this->lev['uri']->param('redirect')) {
    ...
    /**
     * Handles updating Lev
     *
     * Route: GET /update.json/task:updatelev (AJAX call)
     *
     * @return bool False if user has no permissions.
     */
    public function taskUpdatelev()
    {
        if (!$this->authorizeTask('install lev', ['admin.super'])) {
            $this->admin->json_response = [
                'status'  => 'error',
                'message' => $this->admin::translate('PLUGIN_ADMIN.INSUFFICIENT_PERMISSIONS_FOR_TASK')
            ];
        ...
        if ($result) {
            $json_response = [
                ...
                'type'    => 'updatelev',
                'version' => $version,
                'message' => $this->admin::translate('PLUGIN_ADMIN.LEV_WAS_SUCCESSFULLY_UPDATED_TO') . ' ' . $version
            ];
        } else {
            $json_response = [
                'status'  => 'error',
                ...
                'message' => $this->admin::translate('PLUGIN_ADMIN.LEV_UPDATE_FAILED') . ' <br>' . Installer::lastErrorMsg()
            ];
        }
    ...
    protected function taskGpmRelease()
    {
        ...
        if ($current_release !== $release) {
            ...
            $file = CompiledYamlFile::instance($this->lev['locator']->findResource('config://system.yaml'));
            $data->file($file);
    ...
    protected function taskGetUpdates()
    {
        ...
        if (isset($this->lev['session'])) {
            $this->lev['session']->close();
        }

        try {
            $gpm = new LevGPM($flush);
            ...
            if ($gpm->lev !== null) {
                $lev_updates = [
                    'isUpdatable' => $gpm->lev->isUpdatable(),
                    'assets'      => $gpm->lev->getAssets(),
                    'version'     => LEV_GVERSION,
                    'available'   => $gpm->lev->getVersion(),
                    'date'        => $gpm->lev->getDate(),
                    'isSymlink'   => $gpm->lev->isSymlink()
                ];
    ...
    public function taskSaveNewFolder()
    {
        ...
        if ($data['route'] === '' || $data['route'] === '/') {
            $path = $this->lev['locator']->findResource('page://');
        } else {
        ...
        $this->lev->fireEvent('onAdminAfterSaveAs', new Event(['path' => $new_path]));
    ...
    protected function savePage()
    {
        ...
        $data = (array)$this->data;
        $this->lev['twig']->twig_vars['current_form_data'] = $data;
        ...
        $xss_whitelist = $this->lev['config']->get('security.xss_whitelist', 'admin.super');
        ...
        if ($path !== '') {
            ...
            $parent = $pages->get(LEV_SITE_DIR . '/' . $path);
            if (!$parent) {
        ...
        if ($obj) {
            // Event to manipulate data before saving the object
            // DEPRECATED: page
            $this->lev->fireEvent('onAdminSave', new Event(['object' => &$obj, 'page' => &$obj]));
            ...
            $this->admin->setMessage($this->admin::translate('PLUGIN_ADMIN.SUCCESSFULLY_SAVED'), 'info');
            // DEPRECATED: page
            $this->lev->fireEvent('onAdminAfterSave', new Event(['object' => $obj, 'page' => $obj]));
        }
    ...
    protected function taskCopy()
    {
        ...
        try {
            ...
            $page->path($page->parent()->path() . '/' . $page->order() . $folder);
            ...
            // DEPRECATED: page
            $this->lev->fireEvent('onAdminAfterSave', new Event(['object' => $page, 'page' => $page]));
    ...
    protected function taskDelete()
    {
        ...
        try {
            ...
            // DEPRECATED: page
            $this->lev->fireEvent('onAdminAfterDelete', new Event(['object' => $page, 'page' => $page]));
    ...
    protected function taskSwitchlanguage()
    {
        ...
        $language = $data['lang'] ?? $this->lev['uri']->param('lang');
        ...
        if ($language) {
            $this->lev['session']->admin_lang = $language ?: 'en';
        }
    ...
    protected function taskSaveas()
    {
        ...
        if ($lang) {
            $this->lev['session']->admin_lang = $lang ?: 'en';
        }
        ...
        if ($file) {
            ...
            // DEPRECATED: page
            $this->lev->fireEvent('onAdminSave', new Event(['object' => $aPage, 'page' => &$aPage]));
            ...
            // DEPRECATED: page
            $this->lev->fireEvent('onAdminAfterSave', new Event(['object' => $aPage, 'page' => $aPage]));
        }
    ...
    public function taskContinue()
    {
        ...
        if (null !== $folder && 0 === strpos($folder, '@slugify-')) {
            $folder = \Lev\Plugin\Admin\Utils::slug($data[substr($folder, 9)] ?? '');
        }
        if (!$folder) {
            $folder = \Lev\Plugin\Admin\Utils::slug($title) ?: '';
        }
    ...
    /**
     * $data['route'] = $this->lev['uri']->param('route');
     * $data['sortby'] = $this->lev['uri']->param('sortby', null);
     * $data['filters'] = $this->lev['uri']->param('filters', null);
     * $data['page'] $this->lev['uri']->param('page', true);
     * $data['base'] = $this->lev['uri']->param('base');
     * $initial = (bool) $this->lev['uri']->param('initial');
     *
            ...
            foreach ($collection as $page) {
                foreach ($queries as $query) {
                    ...
                    if (stripos($page->getRawContent(), $query) === false
                        ...
                        && stripos($page->slug(), \Lev\Plugin\Admin\Utils::slug($query)) === false
                    ) {
    ...
    protected function taskProcessMarkdown()
    {
        ...
        try {
            ...
            $template_paths = $this->lev['locator']->findResources('theme://templates');
            $this->lev['twig']->twig->getLoader()->addLoader(new FilesystemLoader($template_paths));
    ...
    protected function taskAddmedia()
    {
        ...
        $lev_limit = Utils::getUploadLimit();
        if ($lev_limit > 0 && $_FILES['file']['size'] > $lev_limit) {
            $this->admin->json_response = [
                'status'  => 'error',
                'message' => $this->admin::translate('PLUGIN_ADMIN.EXCEEDED_LEV_FILESIZE_LIMIT')
            ];
        ...
        $include_metadata = Lev::instance()['config']->get('system.media.auto_metadata_exif', false);
        ...
        // DEPRECATED: page
        $this->lev->fireEvent('onAdminAfterAddMedia', new Event(['object' => $page, 'page' => $page]));
    ...
    protected function taskCompileScss()
    {
        ...
        $default_scheme = $this->lev['config']->get('plugins.admin.whitelabel.color_scheme');
        ...
        [$compile_status, $msg] = $this->lev['admin-whitelabel']->compilePresetScss($data, $options);
    ...
    protected function taskExportScss()
    {
        ...
        $name = empty($this->data['whitelabel']['color_scheme']['name']) ? 'admin-theme-export' : \Lev\Plugin\Admin\Utils::slug($this->data['whitelabel']['color_scheme']['name']);

        $location  = 'asset://' . $name . '.yaml';

        [$status, $msg] = $this->lev['admin-whitelabel']->exportPresetScsss($data, $location);
    ...
    protected function taskDelmedia()
    {
        ...
        // DEPRECATED: page
        $this->lev->fireEvent('onAdminAfterDelMedia', new Event(['object' => $page, 'page' => $page, 'media' => $media, 'filename' => $filename]));
    ...
    protected function getLevelListing($data)
    {
        ...
        if (!$route) {
            $is_page = false;
            $route = $this->lev['locator']->findResource('page://', true);
            $root = true;
        }
        ...
        if ($is_page) {
            ...
            $page = $pages->get(LEV_SITE_DIR . $route);
            ...
            // Try a physical path
            if (!Utils::startsWith($route, LEV_SITE_DIR)) {
                $try_path = LEV_SITE_DIR . $route;
            } else {
                $try_path = $route;
            }
        ...
        if ($path) {
            ...
            foreach (new \DirectoryIterator($path) as $fileInfo) {
                ...
                if ($fileInfo->isDot()) {
                    ...
                    $file_path = Utils::replaceFirstOccurrence(LEV_SITE_DIR, '', $filePath);
    ...
    public function getMedia(PageInterface $page = null)
    {
        ...
        $this->uri = $this->uri ?? $this->lev['uri'];
        ...
        if (isset($input['name']) && !empty($input['name'])) {
            ...
            if ($language->enabled()) {
                ...
                if ($languageCode) {
                    ...
                    $includeLang = !$isDefault || (bool)$this->lev['config']->get('system.languages.include_default_lang_file_extension', true);
                    if (!$includeLang) {
    ...
    public function determineFilenameIncludingLanguage($current_filename, $language)
    {
        ...
        $languages_enabled = $this->lev['config']->get('system.languages.supported', []);
