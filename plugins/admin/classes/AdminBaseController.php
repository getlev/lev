...
class AdminBaseController
{
    ...
    public function execute()
    {
        ...
        if (method_exists($this, $method)) {
            ...
            $response = $this->lev->fireEvent('onAdminTaskExecute',
    ...
    protected function validateNonce()
    {
        if (strtolower($_SERVER['REQUEST_METHOD']) === 'post') {
            if (isset($this->post['admin-nonce'])) {
                ...
                $nonce = $this->lev['uri']->param('admin-nonce');
            }
            ...
            if ($this->task === 'logout') {
                $nonce = $this->lev['uri']->param('logout-nonce');
    ...
    public function taskFilesUpload()
    {
        ...
        // Set destination
        if ($this->lev['locator']->isStream($settings->destination)) {
            $destination = $this->lev['locator']->findResource($settings->destination, false, true);
        } else {
    ...
    public function authorizeTask($task = '', $permissions = [])
    {
        if (!$this->admin->authorize($permissions)) {
            if ($this->lev['uri']->extension() === 'json') {
                $this->admin->json_response = [
                    'status'  => 'unauthorized',
    ...
    protected function storeFiles($obj)
    {
        ...
        if (is_array($queue)) {
            $queue = $queue[base64_encode($this->lev['uri']->url())];
    ...
    protected function taskRemoveFileFromBlueprint()
    {
        ...
        if ($type === 'pages' || strpos($blueprint, 'pages/') === 0) {
            ...
            if ($type === 'themes' || $type === 'plugins') {
                $obj = $this->lev[$type]->get(Utils::substrToString($blueprint, '/')); //here
        ...
        if ($page) {
            ...
            $files            = $this->lev['config']->get($blueprint_field);
            ...
            $this->lev['config']->set($blueprint_field, $files);

            switch ($type) {
                case 'config':
                    $data   = $this->lev['config']->get($blueprint_name);
