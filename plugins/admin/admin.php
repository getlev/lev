...
use Lev\Common\Processors\Events\RequestHandlerEvent;
...
use Lev\Framework\Session\Exceptions\SessionException;
use Lev\Plugin\Admin\Admin;
use Lev\Plugin\Admin\AdminFormFactory;
use Lev\Plugin\Admin\Popularity;
use Lev\Plugin\Admin\Router;
use Lev\Plugin\Admin\Themes;
use Lev\Plugin\Admin\AdminController;
use Lev\Plugin\Admin\Twig\AdminTwigExtension;
use Lev\Plugin\Admin\WhiteLabel;
use Lev\Plugin\Form\Form;
use Lev\Plugin\Form\Forms;
...
class AdminPlugin extends Plugin
{
    ...
    public function onRequestHandlerInit(RequestHandlerEvent $event)
    {
        ...
        if ($base === $this->base) {
            ...
            $event->addMiddleware('admin_router', new Router($this->lev, $this->admin));
        }
    }
    ...
    public function onAdminSave(Event $event)
    {
        ...
        if ($obj instanceof Data
            ...
            [$status, $msg] = $this->lev['admin-whitelabel']->compilePresetScss($obj);
            if (!$status) {
                $this->lev['messages']->add($msg, 'error');
            }
        }
    }
    ...
    {
        $lang = $this->lev['language'];
        $this->lev['twig']->plugins_hooked_dashboard_widgets_top[] = [
        ...
        $this->lev['twig']->plugins_hooked_dashboard_widgets_main[] = [
    ...
    public function onPagesInitialized()
    {
        ...
        if (isset($adminParams['controller'])) {
            ...
            /** @var \Lev\Plugin\Admin\Controllers\AdminController $controller */
            $controller = new $class($this->lev);
        ...
        if (empty($this->lev['page'])) {
            $this->lev['page'] = function () use ($user) {
                $page = new Page();
                ...
                if ($user->authorize('admin.login')) {
                    ...
                    $event = $this->lev->fireEvent('onAdminPage', $event);
        ...
        if (empty($this->lev['page'])) {
            if ($user->authenticated) {
                ...
                $event->page = null;
                $event = $this->lev->fireEvent('onPageNotFound', $event);
                /** @var PageInterface $page */
                $page = $event->page;

                if (!$page || !$page->routable()) {
                    Admin::DEBUG && Admin::addDebugMessage('Admin page: 404 Not Found');
                    $error_file = $this->lev['locator']->findResource('plugins://admin/pages/admin/error.md');
                    $page = new Page();
                ...
                $login_file = $this->lev['locator']->findResource('plugins://admin/pages/admin/login.md');
                $page = new Page();
        ...
        if ($legacyController) {
            // Handle tasks.
            $this->admin->task = $task = $this->lev['task'] ?? $this->lev['action'];
            if ($task) {
                Admin::DEBUG && Admin::addDebugMessage("Admin task: {$task}");

                // Make local copy of POST.
                $post = $this->lev['uri']->post();
        ...
        $this->lev['assets']->setTimestamp(substr(md5(LEV_GVERSION . $this->lev['config']->checksum()), 0, 10));
    }
    ...
    {
        $twig_paths = [];
        $this->lev->fireEvent('onAdminTwigTemplatePaths', new Event(['paths' => &$twig_paths]));

        $twig_paths[] = __DIR__ . '/themes/' . $this->theme . '/templates';

        $this->lev['twig']->twig_paths = $twig_paths;
    ...
    public function onTwigSiteVariables()
    {
        ...
        $theme_url = '/' . ltrim($this->lev['locator']->findResource('plugins://admin/themes/' . $this->theme,
            false), '/');
        $twig->twig_vars['theme_url'] = $theme_url;
        $twig->twig_vars['base_url'] = $twig->twig_vars['base_url_relative'];
        $twig->twig_vars['base_path'] = LEV_SITE_DIR;
        ...
        if ($custom_css) {
            $this->lev['assets']->addInlineCss($custom_css);
        }
        // End white label functionality

        $fa_icons_file = CompiledYamlFile::instance($this->lev['locator']->findResource('plugins://admin/themes/lev/templates/forms/fields/iconpicker/icons' . YAML_EXT));
        ...
        $this->lev['twig']->plugins_hooked_nav = [];
        $this->lev->fireEvent('onAdminMenu');
        uasort($this->lev['twig']->plugins_hooked_nav, function ($a, $b) {
            $ac = $a['priority'] ?? 0;
            $bc = $b['priority'] ?? 0;
        ...
        $this->lev->fireEvent('onAdminDashboard');
        ...
        $flashData = $this->lev['session']->getFlashCookieObject(Admin::TMP_COOKIE_NAME);
    ...
    {
        $actions = PermissionsReader::fromYaml("plugins://{$this->name}/permissions.yaml");
    ...
    public static function pagesTypes(bool $keysOnly = false)
    {
        ...
        $hideTypes = (array)Lev::instance()['config']->get('plugins.admin.hide_page_types');
        ...
        Lev::instance()->fireEvent('onAdminPageTypes', $e);
    ...
    public static function pagesModularTypes(bool $keysOnly = false)
    {
        ...
        // First filter by configuration
        $hideTypes = (array)Lev::instance()['config']->get('plugins.admin.hide_modular_page_types');
        foreach ($hideTypes as $hide) {
            if (isset($types[$hide])) {
                unset($types[$hide]);
            } else {
        ...
        // Allow manipulating of the data by event
        $e = new Event(['types' => &$types]);
        Lev::instance()->fireEvent('onAdminModularPageTypes', $e);
    ...
    protected function initializeController($task, $post = null): void
    {
        ...
        $controller->initialize($this->lev, $this->template, $task, $this->route, $post);
        $controller->execute();
        $controller->redirect();
    }
    ...
    protected function setupAdmin()
    {
        ...
        try {
            ...
            $this->lev['messages']->add($message, 'error');
        }
    ...
    protected function initializeAdmin()
    {
        ...
        if (!$this->lev['config']->get('plugins.login.enabled') || !$this->lev['config']->get('plugins.form.enabled') || !$this->lev['config']->get('plugins.email.enabled')) {
        ...
        if ($this->uri->param('task') !== 'processmarkdown') {
            $this->lev['twig']->setAutoescape(true);
        }
        ...
        // Initialize admin class (also registers it to Lev services).
        $this->admin = new Admin($this->lev, $this->admin_route, $this->template, $this->route);

        // Get theme for admin
        $this->theme = $this->config->get('plugins.admin.theme', 'lev');

        // Replace themes service with admin.
        $this->lev['themes'] = function () {
            return new Themes($this->lev);
        };

        // Initialize white label functionality
        $this->lev['admin-whitelabel'] = new WhiteLabel();

        // Compile a missing preset.css file
        $preset_css = 'asset://admin-preset.css';
        $preset_path = $this->lev['locator']->findResource($preset_css);
        if (!$preset_path) {
            $this->lev['admin-whitelabel']->compilePresetScss($this->config->get('plugins.admin.whitelabel'));
        }
        ...
        $config_path = $locator->findResource('app://config');
        ...
        $assets = $this->lev['assets'];
        $translations = 'this.LevAdmin = this.LevAdmin || {}; if (!this.LevAdmin.translations) this.LevAdmin.translations = {}; ' . PHP_EOL . 'this.LevAdmin.translations.PLUGIN_ADMIN = {';
        ...
        $translations .= 'this.LevAdmin.translations.PLUGIN_FORM = {';
        ...
        $translations .= 'this.LevAdmin.translations.LEV_CORE = {';
        $strings = [
            'NICETIME.SECOND',
            'NICETIME.MINUTE',
            'NICETIME.HOUR',
        ...
        foreach ($strings as $string) {
            $separator = (end($strings) === $string) ? '' : ',';
            $translations .= '"' . $string . '": ' . json_encode($this->admin::translate('LEV.'.$string)) . $separator;
        }
        $translations .= '};';
        ...
        $this->lev->fireEvent('onAdminRegisterPermissions', new Event(['admin' => $this->admin]));
    }
    ...
    public static function themeOptions()
    {
        ...
        if (null === $options) {
            $options = [];
            $theme_files = glob(__dir__ . '/themes/lev/css/codemirror/themes/*.css');
    ...
    {
        $filename = $this->lev['locator']->findResource('plugins://admin/presets.yaml', false);
