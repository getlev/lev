...
export const getStore = () => {
    ...
    return JSON.parse(b64_decode_unicode(getCookie('lev-admin-flexpages') || 'e30='));
};
...
export const setStore = (store = {}, options = { expires: '1Y', samesite: 'Lax' }) => {
    ...
    return setCookie('lev-admin-flexpages', b64_encode_unicode(JSON.stringify(store)), options);
};
...
export const ReLoad = (fresh = false) => {
    ...
    const loader = container.querySelector('.lev-loading');
    const content = container.querySelector('#pages-columns');
    const levConfig = typeof global.LevConfig !== 'undefined' ? global.LevConfig : global.LevAdmin.config;
