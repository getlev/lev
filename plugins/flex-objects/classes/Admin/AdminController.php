...
namespace Lev\Plugin\FlexObjects\Admin;
...
use Lev\Common\Flex\Types\Pages\PageCollection;
use Lev\Common\Flex\Types\Pages\PageIndex;
use Lev\Common\Flex\Types\Pages\PageObject;
use Lev\Common\Lev;
...
use Lev\Framework\File\Formatter\CsvFormatter;
use Lev\Framework\File\Formatter\YamlFormatter;
use Lev\Framework\File\Interfaces\FileFormatterInterface;
use Lev\Framework\Flex\FlexForm;
use Lev\Framework\Flex\FlexFormFlash;
...
use Lev\Framework\Flex\Interfaces\FlexDirectoryInterface;
use Lev\Framework\Flex\Interfaces\FlexFormInterface;
use Lev\Framework\Flex\Interfaces\FlexObjectInterface;
use Lev\Framework\Flex\Interfaces\FlexTranslateInterface;
use Lev\Framework\Object\Interfaces\ObjectInterface;
...
use Lev\Plugin\FlexObjects\Controllers\MediaController;
use Lev\Plugin\FlexObjects\Flex;
...
class AdminController
{
    ...
    public function taskDefault(): void
    {
        ...
        if ($object && $object->exists()) {
            ...
            try {
                $this->lev->fireEvent('onFlexTask' . ucfirst($this->task), $event);
            } catch (Exception $e) {
    ...
    public function actionDefault(): void
    {
        ...
        if ($object && $object->exists()) {
            ...
            try {
                $this->lev->fireEvent('onFlexAction' . ucfirst($this->action), $event);
            } catch (Exception $e) {
                /** @var Debugger $debugger */
                $debugger = $this->lev['debugger'];
                $debugger->addException($e);
    ...
    public function taskDelete(): void
    {
        ...
        try {
            ...
            if ($object && $object->exists()) {
                ...
                if ($this->currentRoute->withoutLevParams()->getRoute() === $this->referrerRoute->getRoute()) {
                    $redirect = dirname($this->currentRoute->withoutLevParams()->toString(true));
                } else {
                ...
                $this->lev->fireEvent('onFlexAfterDelete', new Event(['type' => 'flex', 'object' => $object]));
            }
    ...
    public function taskSaveNewFolder(): void
    {
        ...
        if ($locator->isStream($new_path)) {
            ...
            $new_path = LEV_SITE_DIR . '/' . $new_path;
        }
    ...
    protected function continuePages(FlexDirectoryInterface $directory): void
    {
        ...
        if (null !== $folder && 0 === strpos($folder, '@slugify-')) {
            $folder = \Lev\Plugin\Admin\Utils::slug($this->data[substr($folder, 9)] ?? '');
        }
        if (!$folder) {
        ...
        try {
            ...
            if (!$this->redirect) {
                ...
                if ($postAction === 'create-new') {
                    // Create another.
                    $route = $this->referrerRoute->withLevParam('action', null)->withLevParam('', 'add');
                } elseif ($postAction === 'list') {
                    // Back to listing.
                    $route = $this->currentRoute;

                    // Remove :add action.
                    $actionAdd = $key === '' || $route->getLevParam('action') === 'add' || $route->getLevParam('') === 'add';
                    if ($actionAdd) {
                        $route = $route->withLevParam('action', null)->withLevParam('', null);
                    }
            ...
            $this->lev->fireEvent('onFlexAfterSave', new Event(['type' => 'flex', 'object' => $object]));
        } catch (RuntimeException $e) {
    ...
    protected function forwardMediaTask(string $type, string $name): ResponseInterface
    {
        ...
        $route = Uri::getCurrentRoute()->withLevParam('task', null);
        $object = $this->getObject();
    ...
    {
        return Lev::instance()['flex_objects'];
    }
    ...
    public function __construct()
    {
        ...
        [, $location, $target] = $this->lev['admin']->getRouteDetails();
        if (!$location) {
            return;
        }
        ...
        if ($directory) {
            ...
            if (isset($test['redirect'])) {
                ...
                $routeObject = $routeObject->withLevParam('', $test['action']);
            }
        ...
        $this->action = $this->post['action'] ?? $uri->param('action', null) ?? $uri->param('', null) ?? $routeObject->getLevParam('');
        $this->active = true;
        $this->currentRoute = $uri::getCurrentRoute();
        $this->route = $routeObject;

        $base = $this->lev['pages']->base();
        if ($base) {
    ...
    public function execute()
    {
        ...
        $this->lev->fireEvent("flex.{$this->target}.admin.route", $event);
    ...
    public function getDirectory(string $type = null): ?FlexDirectoryInterface
    {
        ...
        if ($type && null === $this->directory) {
            $this->directory = Lev::instance()['flex_objects']->getDirectory($type);
        }
    ...
    protected function validateNonce(): bool
    {
        ...
        $nonce = $this->post[$this->nonce_name] ?? $this->lev['uri']->param($this->nonce_name) ?? $this->lev['uri']->query($this->nonce_name);

        if (!$nonce) {
            $nonce = $this->post['admin-nonce'] ?? $this->lev['uri']->param('admin-nonce') ?? $this->lev['uri']->query('admin-nonce');
