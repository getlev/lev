...
abstract class AbstractController implements RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        $this->lev = $attributes['lev'] ?? Lev::instance();
        ...
        try {
            ...
            $this->lev->fireEvent("flex.{$this->type}.{$type}.{$command}", $event);
    ...
    protected function getFormFlash(FlexObjectInterface $object, string $type = '')
    {
        ...
        if (!$flash->exists()) {
            $flash->setUrl($url)->setUser($this->lev['user']);
        }
    ...
    protected function checkNonce(string $task): void
    {
        ...
        if (!$nonce) {
            $nonce = $this->lev['uri']->param($this->nonce_name);
        }

        if (!$nonce) {
            $nonce = $this->lev['uri']->query($this->nonce_name);
        }
