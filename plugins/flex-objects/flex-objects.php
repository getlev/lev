...
use Lev\Plugin\FlexObjects\Controllers\ObjectController;
use Lev\Plugin\FlexObjects\FlexFormFactory;
use Lev\Plugin\Form\Forms;
use Lev\Plugin\FlexObjects\Admin\AdminController;
...
{
    /** @var string */
    protected const MIN_LEV_GVERSION = '1.7.0';
    ...
    {
        return version_compare(LEV_GVERSION, static::MIN_LEV_GVERSION, '>=');
    }
    ...
    public function initializeFlex(): void
    {
        ...
        // Add to DI container
        $this->lev['flex_objects'] = static function (Lev $lev) use ($config) {
            ...
            $lev->fireEvent('onFlexInit', new Event(['flex' => $flexObjects]));
    ...
    {
        /** @var \Lev\Framework\Flex\Flex $flex */
        $flex = $event->flex;
    ...
    public function onBeforeFlexFormInitialize(Event $event): void
    {
        ...
        if (!isset($form['flex']['key']) && $edit === true) {
            /** @var Route $route */
            $route = $this->lev['route'];
            $id = $route->getLevParam('id');
            if (null !== $id) {
                $form['flex']['key'] = $id;
                $event['form'] = $form;
            }
    ...
    public function onPagesInitialized(Event $event): void
    {
        ...
        if ($page) {
            ...
            if (\is_string($router)) {
                ...
                $flexEvent = $this->lev->fireEvent("flex.router.{$router}", $flexEvent);
                if ($response) {
    ...
    public function authorizePage(Event $event): void
    {
        ...
        foreach ($forms as $name => $test) {
            ...
            if ($type === 'flex') {
                ...
                $this->lev->fireEvent('onBeforeFlexFormInitialize', new Event(['page' => $page, 'name' => $name, 'form' => &$form]));
        ...
        $type = $form['flex']['type'] ?? $config['directory'] ?? $route->getLevParam('directory') ?? null;
        $key = $form['flex']['key'] ?? $config['id'] ?? $route->getLevParam('id') ?? '';
        if (\is_string($type)) {
            /** @var Flex $flex */
            $flex = $this->lev['flex_objects'];
        ...
        if (!$hasAccess) {
            ...
            if (!$page->isModule()) {
                $login = $this->lev['login'] ?? null;
                $unauthorized = $login ? $login->addPage('unauthorized') : null;
                if ($unauthorized) {
                    unset($this->lev['page']);
                    $this->lev['page'] = $unauthorized;
                }
            }
    ...
    /**
     * @param \Lev\Framework\Flex\Flex $flex
     * @param array $types
     * @param bool $report
     */
    protected function registerDirectories(\Lev\Framework\Flex\Flex $flex, array $types, bool $report = false): void
    {
    ...
    {
        $event['scss']->add($this->lev['locator']->findResource('plugins://flex-objects/scss/_preset.scss'));
    }
    ...
    public function onAdminMenu(): void
    {
        ...
        foreach ($this->getAdminMenu() as $route => $item) {
            ...
            $index = $item['index'] ?? 0;
            if (($this->lev['twig']->plugins_hooked_nav[$title]['index'] ?? 1000) <= $index) {
                continue;
            }
            ...
            $this->lev['twig']->plugins_hooked_nav[$title] = [
                'location' => $location,
                'route' => $route,
                'index' => $index,
                'icon' => $icon,
    ...
    {
        $this->lev['admin']->dataTypesExcludedFromDataManagerPlugin[] = 'flex-objects';
    }
    ...
    public function onTwigTemplatePaths(): void
    {
        ...
        $extra_path = $extra_site_twig_path ? $this->lev['locator']->findResource($extra_site_twig_path) : null;
        if ($extra_path) {
            $this->lev['twig']->twig_paths[] = $extra_path;
        }

        $this->lev['twig']->twig_paths[] = __DIR__ . '/templates';
    }
    ...
    public function onAdminTwigTemplatePaths(Event $event): void
    {
        ...
        $extra_path = $extra_admin_twig_path ? $this->lev['locator']->findResource($extra_admin_twig_path) : null;
    ...
    public function onTwigAdminVariables(): void
    {
        if ($this->controller->isActive()) {
            // Twig shortcuts
            $this->lev['twig']->twig_vars['controller'] = $this->controller;
            $this->lev['twig']->twig_vars['action'] = $this->controller->getAction();
            $this->lev['twig']->twig_vars['task'] = $this->controller->getTask();
            $this->lev['twig']->twig_vars['target'] = $this->controller->getTarget();
            $this->lev['twig']->twig_vars['key'] = $this->controller->getId();

            $this->lev['twig']->twig_vars['flex'] = $this->lev['flex_objects'];
            $this->lev['twig']->twig_vars['directory'] = $this->controller->getDirectory();
            $this->lev['twig']->twig_vars['collection'] = $this->controller->getCollection();
            $this->lev['twig']->twig_vars['object'] = $this->controller->getObject();

            // CSS / JS Assets
            $this->lev['assets']->addCss('plugins://flex-objects/css/admin.css');
            $this->lev['assets']->addCss('plugins://admin/themes/lev/css/codemirror/codemirror.css');
        }
    }
}
