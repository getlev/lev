...
use Lev\Common\Twig\Exception\TwigException;
use Lev\Common\Twig\Extension\FilesystemExtension;
use Lev\Common\Twig\Extension\LevExtension;
...
class Twig
{
    ...
    public function init()
    {
        if (null === $this->twig) {
            ...
            $this->lev->fireEvent('onTwigTemplatePaths');

            // Add Lev core templates location
            ...
            $this->lev->fireEvent('onTwigLoader');
            ...
            if (empty($params['autoescape'])) {
                user_error('Lev 2.0 will have Twig auto-escaping forced on (can be emulated by turning off \'system.strict_mode.twig_compat\' setting in your configuration)', E_USER_DEPRECATED);
            }
                ...
                if ($config->get('system.twig.undefined_functions')) {
                    if (function_exists($name)) {
                        if (!Utils::isDangerousFunction($name)) {
                            user_error("PHP function {$name}() was used as Twig function. This is deprecated in Lev 1.7. Please add it to system configuration: `system.twig.safe_functions`", E_USER_DEPRECATED);
            ...
            $this->twig->registerUndefinedFilterCallback(function (string $name) use ($config) {
                ...
                if ($config->get('system.twig.undefined_filters')) {
                    if (function_exists($name)) {
                        if (!Utils::isDangerousFunction($name)) {
                            user_error("PHP function {$name}() used as Twig filter. This is deprecated in Lev 1.7. Please add it to system configuration: `system.twig.safe_filters`", E_USER_DEPRECATED);
            ...
            $this->lev->fireEvent('onTwigInitialized');
            ...
            $this->twig->addExtension(new LevExtension());
            ...
            $this->lev->fireEvent('onTwigExtensions');
            ...
            $this->twig_vars += [
                    ...
                    'browser'           => $this->lev['browser'],
                    'base_dir'          => LEV_SITE_DIR,
                    ...
                    'base_url_simple'   => $this->lev['base_url'],
                    'theme_dir'         => $locator->findResource('theme://'),
                    'theme_url'         => $this->lev['base_url'] . '/' . $locator->findResource('theme://', false),
                    'html_lang'         => $this->lev['language']->getActive() ?: $config->get('site.default_lang', 'en'),
    ...
    public function processPage(PageInterface $item, $content = null)
    {
        ...
        $this->lev->fireEvent('onTwigPageVariables', new Event(['page' => $item]));
    ...
    {
        // override the twig header vars for local resolution
        $this->lev->fireEvent('onTwigTemplateVariables');
        $vars += $this->twig_vars;
    ...
    {
        // override the twig header vars for local resolution
        $this->lev->fireEvent('onTwigStringVariables');
        $vars += $this->twig_vars;
    ...
    public function processSite($format = null, array $vars = [])
    {
        try {
            ...
            $lev->fireEvent('onTwigSiteVariables');
            ...
            $twig_vars = $this->twig_vars;
            $twig_vars['theme'] = $lev['config']->get('theme');
            ...
            $params = $lev['uri']->params(null, true);
            if (!empty($params)) {
            ...
            if ($prev instanceof TwigException) {
                ...
                $event = $lev->fireEvent("onDisplayErrorPage.{$code}", $event);
                $newPage = $event['page'];
    ...
    {
        if (!$state) {
            user_error(__CLASS__ . '::' . __FUNCTION__ . '(false) is deprecated since Lev 1.5', E_USER_DEPRECATED);
        }
