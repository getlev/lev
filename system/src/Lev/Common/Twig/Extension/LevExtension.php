...
namespace Lev\Common\Twig\Extension;
...
use Lev\Common\Scheduler\Cron;
use Lev\Common\Security;
use Lev\Common\Twig\TokenParser\TwigTokenParserCache;
use Lev\Common\Twig\TokenParser\TwigTokenParserLink;
use Lev\Common\Twig\TokenParser\TwigTokenParserRender;
use Lev\Common\Twig\TokenParser\TwigTokenParserScript;
use Lev\Common\Twig\TokenParser\TwigTokenParserStyle;
use Lev\Common\Twig\TokenParser\TwigTokenParserSwitch;
use Lev\Common\Twig\TokenParser\TwigTokenParserThrow;
use Lev\Common\Twig\TokenParser\TwigTokenParserTryCatch;
use Lev\Common\Twig\TokenParser\TwigTokenParserMarkdown;
use Lev\Common\User\Interfaces\UserInterface;
use Lev\Common\Utils;
use Lev\Common\Yaml;
use Lev\Common\Helpers\Base32;
...
/**
 * Class LevExtension
 * @package Lev\Common\Twig\Extension
 */
class LevExtension extends AbstractExtension implements GlobalsInterface
{
    /** @var Lev */
    protected $lev;
    /** @var Debugger|null */
    ...
    /**
     * LevExtension constructor.
     */
    public function __construct()
    {
        $this->lev     = Lev::instance();
        $this->debugger = $this->lev['debugger'] ?? null;
    ...
    {
        if (empty($date)) {
            return $this->lev['language']->translate('LEV.NICETIME.NO_DATE_PROVIDED');
        }
        ...
        if (empty($unix_date)) {
            return $this->lev['language']->translate('LEV.NICETIME.BAD_DATE');
        }

        // is it future date or past date
        if ($now > $unix_date) {
            $difference = $now - $unix_date;
            $tense      = $this->lev['language']->translate('LEV.NICETIME.AGO');
        } elseif ($now == $unix_date) {
            $difference = $now - $unix_date;
            $tense      = $this->lev['language']->translate('LEV.NICETIME.JUST_NOW');
        } else {
            $difference = $unix_date - $now;
            $tense      = $this->lev['language']->translate('LEV.NICETIME.FROM_NOW');
        }
        ...
        if ($this->lev['language']->getTranslation(
            $this->lev['language']->getLanguage(),
        ...
        $periods[$j] = $this->lev['language']->translate('LEV.'.$periods[$j]);
    ...
    public function translate(Environment $twig, ...$args)
    {
        ...
        if (isset($this->lev['admin'])) {
            ...
            return $this->lev['admin']->translate($args, $lang);
        }

        // else use the default lev translate functionality
        return $this->lev['language']->translate($args);
    }
    ...
    {
        return $this->lev['language']->translate(func_get_args());
    }
    ...
    public function exifFunc($image, $raw = false)
    {
        if (isset($this->lev['exif'])) {
            ...
            $exif_reader = $this->lev['exif']->getReader();
    ...
    {
        $page = $page ?? $context['page'] ?? Lev::instance()['page'] ?? null;
        ...
        return Lev::instance()['config']->get('theme.' . $var, $default);
    }
