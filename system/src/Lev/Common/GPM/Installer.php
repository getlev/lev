...
class Installer
{
    ...
    /** @const Target is not a Lev instance */
    public const NOT_LEV_SITE_DIR = 16;
     ...
     * @param  string $destination The local path to the Lev Instance
    ...
    public static function install($zip, $destination, $options = [], $extracted = null, $keepExtracted = false)
    {
        ...
        $install_path = rtrim($destination . '/' . ltrim($options['install_path'], '/'), '/');

        if (!self::isLevInstance($destination) || !self::isValidDestination(
            $install_path,
    ...
    {
        $installer_file_folder = rtrim($installer_file_folder, '/');
        ...
        if ($is_install) {
            $slug = '';
            if (($pos = strpos($installer_file_folder, 'lev-plugin-')) !== false) {
                $slug = substr($installer_file_folder, $pos + strlen('lev-plugin-'));
            } elseif (($pos = strpos($installer_file_folder, 'lev-theme-')) !== false) {
                $slug = substr($installer_file_folder, $pos + strlen('lev-theme-'));
            }
        } else {
    ...
    public static function sophisticatedInstall($source_path, $install_path, $ignores = [], $keep_source = false)
    {
        foreach (new DirectoryIterator($source_path) as $file) {
            ...
            $path = $install_path . '/' . $file->getFilename();
    ...
    /**
     * Validates if the given path is a Lev Instance
     *
     * @param  string $target The local path to the Lev Instance
     * @return bool True if is a Lev Instance. False otherwise
     */
    public static function isLevInstance($target)
    {
        self::$error = 0;
        self::$target = $target;

        if (!file_exists($target . '/' . 'index.php') ||
            !file_exists($target . '/' . 'bin') ||
            !file_exists($target . '/' . 'user') ||
            !file_exists($target . '/' . 'system' . '/' . 'config' . '/' . 'system.yaml')
        ) {
            self::$error = self::NOT_LEV_SITE_DIR;
        }
    ...
    public static function lastErrorMsg()
    {
        ...
        switch (self::$error) {
            ...
            case self::NOT_LEV_SITE_DIR:
                $msg = 'The target path "' . self::$target . '" does not appear to be a Lev instance';
                break;
