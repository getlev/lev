...
class GPM extends Iterator
{
    ...
    private $repository;
    /** @var Remote\LevCore|null Remove Lev Packages */
    private $lev;
    /** @var bool */
    private $refresh;
    /** @var callable|null */
    ...
    public function __construct($refresh = false, $callback = null)
    {
        ...
        Folder::create(LEV_CACHE_PATH . '/gpm');
    ...
    public function isPluginEnabled($slug): bool
    {
        ...
        return ($lev['config']['plugins'][$slug]['enabled'] ?? false) === true;
    }
    ...
    public function isThemeEnabled($slug): bool
    {
        ...
        $current_theme = $lev['config']['system']['pages']['theme'] ?? null;
    ...
    /**
     * Returns Lev version available in the repository
     *
     * @return Remote\LevCore|null
     */
    public function getLev()
    {
        if (null === $this->lev) {
            try {
                $this->lev = new Remote\LevCore($this->refresh, $this->callback);
            } catch (Exception $e) {}
        }
    ...
    public function findPackage($search, $ignore_exception = false)
    {
        ...
        if (null === $themes || null === $plugins) {
            if (!is_writable(LEV_SITE_DIR . '/cache/gpm')) {
            ...
            throw new RuntimeException('GPM not reachable. Please check your internet connection or check the Lev site is reachable');
        }
    ...
    public static function downloadPackage($package_file, $tmp)
    {
        ...
        if (Lev::instance()['config']->get('system.gpm.official_gpm_only') && ($package['host'] ?? null) !== 'getgrav.org') {
    ...
    public static function copyPackage($package_file, $tmp)
    {
        ...
        if ($package_file && file_exists($package_file)) {
            ...
            Folder::create($tmp);
            copy($package_file, $tmp . '/' . $filename);
            return $tmp . '/' . $filename;
        }
    ...
    public static function getInstallPath($type, $name)
    {
        ...
        if ($type === 'theme') {
            $install_path = $locator->findResource('themes://', false) . '/' . $name;
        } else {
            $install_path = $locator->findResource('plugins://', false) . '/' . $name;
        }
    ...
    public function getDependencies($packages)
    {
        ...
        foreach ($dependencies as $dependency_slug => $dependencyVersionWithOperator) {
            ...
            //First, check for Lev dependency. If a dependency requires Lev > the current version, abort and tell.
            if ($dependency_slug === 'lev') {
                $testVersion = $this->calculateVersionNumberFromDependencyVersion($dependencyVersionWithOperator);
                if (version_compare($testVersion, LEV_GVERSION) === 1) {
                    //Needs a Lev update first
                    throw new RuntimeException("<red>One of the packages require Lev {$dependencies['lev']}. Please update Lev to the latest release.");
                }
            ...
            if ($this->isPluginInstalled($dependency_slug)) {
                ...
                $locator = Lev::instance()['locator'];
                $blueprints_path = $locator->findResource('plugins://' . $dependency_slug . '/' . 'blueprints.yaml');
