...
class SessionServiceProvider implements ServiceProviderInterface
{
    ...
    public function register(Container $container)
    {
        ...
        $container['session'] = static function ($c) {
            ...
            $session_prefix = $c['inflector']->hyphenize($config->get('system.session.name', 'lev-site'));
            $session_uniqueness = $config->get('system.session.uniqueness', 'path') === 'path' ?  substr(md5(LEV_SITE_DIR), 0, 7) :  md5($config->get('security.salt'));
