...
namespace Lev\Common\Flex\Types\Pages;

use Lev\Common\Data\Blueprint;
use Lev\Common\Flex\Traits\FlexLevTrait;
use Lev\Common\Flex\Traits\FlexObjectTrait;
use Lev\Common\Lev;
use Lev\Common\Flex\Types\Pages\Traits\PageContentTrait;
use Lev\Common\Flex\Types\Pages\Traits\PageLegacyTrait;
use Lev\Common\Flex\Types\Pages\Traits\PageRoutableTrait;
use Lev\Common\Flex\Types\Pages\Traits\PageTranslateTrait;
...
use Lev\Framework\Object\ObjectCollection;
...
/**
 * Class LevPageObject
 * @package Lev\Plugin\FlexObjects\Types\LevPages
 *
...
class PageObject extends FlexPageObject
{
    ...
    public function initialize(): void
    {
        if (!$this->_initialized) {
            Lev::instance()->fireEvent('onPageProcessed', new Event(['page' => $this]));
    ...
    public function getRoute($query = []): ?Route
    {
        ...
        if ($lang = $route->getLanguage()) {
            $lev = Lev::instance();
            if (!$lev['config']->get('system.languages.include_default_lang')) {
    ...
    public function save($reorder = true)
    {
        ...
        $lev = $this->getContainer();
        if ($fireEvents) {
            $self = $this;
            $lev->fireEvent('onAdminSave', new Event(['type' => 'flex', 'directory' => $this->getFlexDirectory(), 'object' => &$self]));
            if ($self !== $this) {
        ...
        // Backwards compatibility with older plugins.
        if ($fireEvents) {
            $lev->fireEvent('onAdminAfterSave', new Event(['type' => 'flex', 'directory' => $this->getFlexDirectory(), 'object' => $this]));
        }
    ...
    protected function doGetBlueprint(string $name = ''): Blueprint
    {
        try {
            ...
            // TODO: We need to move raw blueprint logic to Lev itself to remove admin dependency here.
            if ($name === 'raw') {
                // Admin RAW mode.
                if ($this->isAdminSite()) {
                    /** @var Admin $admin */
        ...
        if ($isNew === true && $name === '') {
            ...
            Lev::instance()->fireEvent('onBlueprintCreated', new Event(['blueprint' => $blueprint, 'type' => $template]));
        }
