...
namespace Lev\Common\Flex\Types\Pages\Storage;
            ...
            if (is_string($oldKey)) {
                ...
                if ($oldFilename !== $newFilename) {
                    ...
                    if (!$isClone && $file->exists()) {
                        ...
                        $locator = $lev['locator'];
                        $toPath = $locator->isStream($newFilepath) ? $locator->findResource($newFilepath, true, true) : LEV_SITE_DIR . "/{$newFilepath}";
    ...
    protected function getObjectMeta(string $key, bool $reload = false): array
    {
        ...
        if ($reload || !isset($this->meta[$key])) {
            ...
            if (mb_strpos($key, '@@') === false) {
                ...
                if (is_string($path)) {
                    $path = $locator->isStream($path) ? $locator->findResource($path) : LEV_SITE_DIR . "/{$path}";
                } else {
                    $path = null;
                }
            } else {
