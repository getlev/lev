...
use Lev\Framework\Uri\UriPartsFilter;
...
class Uri
{
    ...
    /** @var \Lev\Framework\Uri\Uri|null */
    ...
    public function init()
    {
        ...
        // Handle custom base
        $custom_base = rtrim($lev['config']->get('system.custom_base_url', ''), '/');
        if ($custom_base) {
        ...
        // Set some Lev stuff
        $lev['base_url_absolute'] = $config->get('system.custom_base_url') ?: $this->rootUrl(true);
        $lev['base_url_relative'] = $this->rootUrl(false);
        $lev['base_url'] = $config->get('system.absolute_urls') ? $lev['base_url_absolute'] : $lev['base_url_relative'];
    ...
    {
        return '/\/{1,}([^\:\#\/\?]*' . Lev::instance()['config']->get('system.param_sep') . '[^\:\#\/\?]*)/';
    }
    ...
    public static function ip()
    {
        ...
        } elseif (getenv('HTTP_X_FORWARDED_FOR') && Lev::instance()['config']->get('system.http_x_forwarded.ip')) {
            $ips = array_map('trim', explode(',', getenv('HTTP_X_FORWARDED_FOR')));
            $ip = array_shift($ips);
        } elseif (getenv('HTTP_X_FORWARDED') && Lev::instance()['config']->get('system.http_x_forwarded.ip')) {
    ...
    public static function buildParams(array $params)
    {
        ...
        $lev = Lev::instance();
        $sep = $lev['config']->get('system.param_sep');
    ...
    /**
     * Converts links from absolute '/' or relative (../..) to a Lev friendly format
     *
    ...
    public static function convertUrl(PageInterface $page, $url, $type = 'link', $absolute = false, $route_only = false)
    {
        ...
        $external          = false;
        $base              = $lev['base_url_relative'];
        $base_url          = rtrim($base . $lev['pages']->base(), '/') . $language_append;
        $pages_dir         = $lev['locator']->findResource('page://');
        ...
        if (is_array($url) && !$external && ($absolute === true || $lev['config']->get('system.absolute_urls', false))) {
    ...
    public static function parseUrl($url)
    {
        ...
        [$stripped_path, $params] = static::extractParams($parts['path'], $lev['config']->get('system.param_sep'));
    ...
    public static function convertUrlOld(PageInterface $page, $markdown_url, $type = 'link', $relative = null)
    {
        ...
        if ($relative === null) {
            ...
            $base = $relative ? $lev['base_url_relative'] : $lev['base_url_absolute'];
        }
    ...
    {
        // Build scheme.
        if (isset($env['HTTP_X_FORWARDED_PROTO']) && Lev::instance()['config']->get('system.http_x_forwarded.protocol')) {
        ...
        // Build host.
        if (isset($env['HTTP_X_FORWARDED_HOST']) && Lev::instance()['config']->get('system.http_x_forwarded.host')) {
        ...
        // Build port.
        if (isset($env['HTTP_X_FORWARDED_PORT']) && Lev::instance()['config']->get('system.http_x_forwarded.port')) {
    ...
    public function post($element = null, $filter_type = null)
    {
        if (!$this->post) {
            ...
            Lev::instance()->fireEvent('onHttpPostFilter', $event);
        }
    ...
    /**
     * Check if this is a valid Lev extension
     *
