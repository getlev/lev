...
trait MediaUploadTrait
{
     ...
     *   $settings = ['destination' => 'app://pages/media']; // Settings from the form field.
    ...
    public function checkFileMetadata(array $metadata, string $filename = null, array $settings = null): string
    {
        ...
        if ($filesize) {
            ...
            if ($metadata['size'] > $max_filesize) {
                ...
                throw new RuntimeException($this->translate('PLUGIN_ADMIN.EXCEEDED_LEV_FILESIZE_LIMIT'), 400);
            }
        } elseif (null === $filesize) {
            // Check size against the Lev upload limit.
            $lev_limit = Utils::getUploadLimit();
            if ($lev_limit > 0 && $metadata['size'] > $lev_limit) {
                throw new RuntimeException($this->translate('PLUGIN_ADMIN.EXCEEDED_LEV_FILESIZE_LIMIT'), 400);
            }
        }
