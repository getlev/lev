...
class Plugin implements EventSubscriberInterface, ArrayAccess
{
     ...
     */
    public function __construct($name, Lev $lev, Config $config = null)
    {
        $this->name = $name;
        $this->lev = $lev;
    ...
    protected function isPluginActiveAdmin($plugin_route)
    {
        ...
        $uri = $this->lev['uri'];
        /** @var Config $config */
        $config = $this->config ?? $this->lev['config'];
    ...
    private function getPriority($params, $eventName)
    {
        ...
        return $this->lev['config']->get($override) ?? $params[1] ?? 0;
    }
    ...
    /**
     * Persists to disk the plugin parameters currently stored in the Lev Config object
     *
    ...
    public static function saveConfig($name)
    {
        ...
        $content = $lev['config']->get('plugins.' . $name);
        $file->save($content);
        $file->free();
        unset($file);
    ...
    public static function inheritedConfigOption(string $plugin, string $var, PageInterface $page = null, $default = null)
    {
        if (Utils::isAdminPlugin()) {
            $page = Lev::instance()['admin']->page() ?? null;
        } else {
            $page = $page ?? Lev::instance()['page'] ?? null;
        }
        ...
        return Lev::instance()['config']->get("plugins.$plugin.$var", $default);
    }
