...
/**
 * The LevCache object is used throughout Lev to store and retrieve cached data.
...
class Cache extends Getters
{
     ...
     * @return void
     */
    public function init(Lev $lev)
    {
        $this->config = $lev['config'];
        $this->now = time();
        ...
        $uniqueness = substr(md5($uri->rootUrl(true) . $this->config->key() . LEV_GVERSION), 2, 8);

        // Cache key allows us to invalidate all cache on configuration changes.
        $this->key = ($prefix ?: 'g') . '-' . $uniqueness;
        $this->cache_dir = $lev['locator']->findResource('cache://doctrine/' . $uniqueness, true, true);
        $this->driver_setting = $this->config->get('system.cache.driver');
        $this->driver = $this->getCacheDriver();
        $this->driver->setNamespace($this->key);

        /** @var EventDispatcher $dispatcher */
        $dispatcher = Lev::instance()['events'];
    ...
    public function getSimpleCache()
    {
        if (null === $this->simpleCache) {
            $cache = new \Lev\Framework\Cache\Adapter\DoctrineCache($this->driver, '', $this->getLifetime());
    ...
    /**
     * Helper method to clear all Lev caches
     *
    ...
    public static function clearCache($remove = 'standard')
    {
        ...
        $output = [];
        $user_config = LEV_SITE_DIR . '/' . LEV_APP_PATH . '/' . 'config/system.yaml';
        ...
        switch ($remove) {
            ...
            case 'invalidate':
                ...
                break;
            default:
                if (Lev::instance()['config']->get('system.cache.clear_images_by_default')) {
        ...
        Lev::instance()->fireEvent('onBeforeCacheClear', new Event(['remove' => $remove, 'paths' => &$remove_paths]));
        ...
        Lev::instance()->fireEvent('onAfterCacheClear', new Event(['remove' => $remove, 'output' => &$output]));
    ...
    /**
     * Static function to call as a scheduled Job to clear Lev cache
     *
     * @param string $type
     * @return void
     */
    ...
    public function onSchedulerInitialized(Event $event)
    {
        ...
        $job = $scheduler->addFunction('Lev\Common\Cache::purgeJob', [true], $name);
        $job->at($at);
        $job->output($logs);
        ...
        $job = $scheduler->addFunction('Lev\Common\Cache::clearJob', [$clear_type], $name);
        $job->at($at);
        $job->output($logs);
        $job->backlink('/config/system#caching');
    }
