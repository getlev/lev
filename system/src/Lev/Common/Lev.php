...
use Lev\Common\Processors\AssetsProcessor;
use Lev\Common\Processors\BackupsProcessor;
use Lev\Common\Processors\DebuggerAssetsProcessor;
use Lev\Common\Processors\InitializeProcessor;
use Lev\Common\Processors\PagesProcessor;
use Lev\Common\Processors\PluginsProcessor;
use Lev\Common\Processors\RenderProcessor;
use Lev\Common\Processors\RequestProcessor;
use Lev\Common\Processors\SchedulerProcessor;
use Lev\Common\Processors\TasksProcessor;
use Lev\Common\Processors\ThemesProcessor;
use Lev\Common\Processors\TwigProcessor;
use Lev\Common\Scheduler\Scheduler;
use Lev\Common\Service\AccountsServiceProvider;
use Lev\Common\Service\AssetsServiceProvider;
use Lev\Common\Service\BackupsServiceProvider;
use Lev\Common\Service\ConfigServiceProvider;
use Lev\Common\Service\ErrorServiceProvider;
use Lev\Common\Service\FilesystemServiceProvider;
use Lev\Common\Service\FlexServiceProvider;
use Lev\Common\Service\InflectorServiceProvider;
use Lev\Common\Service\LoggerServiceProvider;
use Lev\Common\Service\OutputServiceProvider;
use Lev\Common\Service\PagesServiceProvider;
use Lev\Common\Service\RequestServiceProvider;
use Lev\Common\Service\SessionServiceProvider;
use Lev\Common\Service\StreamsServiceProvider;
use Lev\Common\Service\TaskServiceProvider;
use Lev\Common\Twig\Twig;
use Lev\Framework\DI\Container;
use Lev\Framework\Psr7\Response;
use Lev\Framework\RequestHandler\RequestHandler;
use Lev\Framework\Route\Route;
use Lev\Framework\Session\Messages;
...
/**
 * Lev container is the heart of Lev.
 *
 * @package Lev\Common
 */
class Lev extends Container
{
    ...
    /**
     * Return the Lev instance. Create it if it's not already instanced
     *
     * @param array $values
     * @return Lev
     */
    public static function instance(array $values = [])
    {
        if (null === self::$instance) {
            ...
            if ($loader) {
                ...
                $loader->addPsr4('Phive\\Twig\\Extensions\\Deferred\\', LEV_SYSTEM_PATH . '/src' . 'Phive/Twig/Extensions/Deferred/', true);
            }
        } elseif ($values) {
    ...
    /**
     * Setup Lev instance using specific environment.
     *
     ...
     *
     * Call after `$lev->setup($environment)`
     *
     * - Load configuration
     * - Initialize logger
     * - Disable debugger
    ...
    public function process(): void
    {
        ...
        if ($response->getHeaderLine('Lev-Internal-SkipShutdown') !== '1') {
     ...
     *
     * Please use $lev->close() and $lev->redirect() instead of calling this one!
     *
     * @return void
     */
    ...
    /**
     * Terminates Lev request with a response.
     *
     ...
     * @param ResponseInterface $response
     * @return never-return
     * @deprecated 1.7 Use $lev->close() instead.
     */
    ...
    /**
     * Terminates Lev request and redirects browser to another location.
     *
    ...
    /**
     * Returns redirect response object from Lev.
     *
     * @param Route|string $route Internal route.
     * @param int|null $code  Redirection code (30x)
    ...
    public function header(ResponseInterface $response = null): void
    {
        ...
        foreach ($response->getHeaders() as $key => $values) {
            // Skip internal Lev headers.
            if (strpos($key, 'Lev-Internal-') === 0) {
                continue;
            }
    ...
    /**
     * Initialize and return a Lev instance
     *
     * @param  array $values
     * @return static
     */
    protected static function load(array $values)
    {
        ...
        $container['lev'] = function (Container $container) {
            user_error('Calling $lev[\'lev\'] or {{ lev.lev }} is deprecated since Lev 1.6, just use $lev or {{ lev }}', E_USER_DEPRECATED);
