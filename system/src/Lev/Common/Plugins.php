...
class Plugins extends Iterator
{
        ...
        foreach ($this->items as $plugin) {
            ...
            if ($config["plugins.{$plugin->name}.enabled"] && $plugin instanceof Plugin) {
                if (isset($plugin->features['blueprints'])) {
                    $blueprints["plugins://{$plugin->name}/blueprints"] = $plugin->features['blueprints'];
                }
    ...
    public function init()
    {
        ...
        $event = new PluginsLoadedEvent($lev, $this);
    ...
    public function __debugInfo(): array
    {
        ...
        unset($array["\0Lev\Common\Iterator\0iteratorUnset"]);
    ...
    public static function get($name)
    {
        ...
        $obj->merge(Lev::instance()['config']->get('plugins.' . $name) ?: []);
    ...
    protected function loadPlugin($name)
    {
        ...
        $file = $locator->findResource('plugins://' . $name . '/' . $name . PLUGIN_EXT);
        if (is_file($file)) {
            // Local variables available in the file: $lev, $name, $file
        ...
        if (null === $class) {
            ...
            $pluginClassFormat = [
                'Lev\\Plugin\\' . ucfirst($name). 'Plugin',
                'Lev\\Plugin\\' . $className . 'Plugin',
                'Lev\\Plugin\\' . $className
            ];

            foreach ($pluginClassFormat as $pluginClass) {
                if (is_subclass_of($pluginClass, Plugin::class, true)) {
                    $class = new $pluginClass($name, $lev);
                    break;
                }
            }
        }
        ...
        if (null === $class) {
            $lev['log']->addWarning(
                sprintf("Plugin '%s' enabled but not found! Try clearing cache with `bin/lev clearcache`", $name)
            );
        }
