...
class Validation
{
    ...
    public static function validate($value, array $field)
    {
        ...
        $message = (string) isset($field['validate']['message'])
            ...
            : $language->translate('LEV.FORM.INVALID_INPUT') . ' "' . $language->translate($name) . '"';
    ...
    public static function checkSafety($value, array $field)
    {
        ...
        if (!static::authorize($xss_whitelist, $user)) {
            ...
            if (is_string($value)) {
                ...
                if ($violation) {
                    $messages[$name][] = $language->translate(['LEV.FORM.XSS_ISSUES', $language->translate($name)], null, true);
                }
    ...
    {
        $format = Lev::instance()['config']->get('system.pages.dateformat.default');
        if ($format) {
