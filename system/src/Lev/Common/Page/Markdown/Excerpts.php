...
namespace Lev\Common\Page\Markdown;

use Lev\Common\Lev;
use Lev\Common\Page\Interfaces\PageInterface;
use Lev\Common\Page\Medium\Link;
...
/**
 * Class Excerpts
 * @package Lev\Common\Page\Markdown
 */
class Excerpts
{
    ...
    public function fireInitializedEvent($markdown): void
    {
        ...
        $lev->fireEvent('onMarkdownInitialized', new Event(['markdown' => $markdown, 'page' => $this->page]));
    }
    ...
    public function processLinkExcerpt(array $excerpt, string $type = 'link'): array
    {
        ...
        if (isset($url_parts['query'])) {
            ...
            $valid_attributes = $lev['config']->get('system.pages.markdown.valid_link_attributes') ?? [];
        ...
        if (!empty($url_parts['scheme']) && !in_array($url_parts['scheme'], ['http', 'https'])) {
            ...
            if ($type === 'link' && $locator->isStream($url)) {
                ...
                $url_parts['path'] = $lev['base_url_relative'] . '/' . $path;
    ...
    public function processImageExcerpt(array $excerpt): array
    {
        ...
        if (!empty($url_parts['stream'])) {
            ...
            $local_file = isset($url_parts['path'])
                ...
                && (empty($url_parts['host']) || $url_parts['host'] === $lev['uri']->host());
            ...
            if ($local_file) {
                ...
                if ($this->page && $folder === $this->page->url(false, false, false)) {
                    ...
                    $base_url = rtrim($lev['base_url_relative'] . $pages->base(), '/');
                    ...
                    if ($ext_page) {
                        ...
                        $lev->fireEvent('onMediaLocate', new Event(['route' => $page_route, 'media' => &$media]));
                    }
                }
            }
        }
