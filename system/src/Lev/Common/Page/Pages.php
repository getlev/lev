...
use Lev\Framework\Flex\Pages\FlexPageObject;
use Lev\Plugin\Admin;
...
class Pages
{
    ...
    public function referrerRoute(?string &$langCode, string $route = '/'): ?string
    {
        ...
        $root = $this->lev['base_url_absolute'];
    ...
    public function baseUrl($lang = null, $absolute = null)
    {
        ...
        return $this->lev[$type] . $this->baseRoute($lang);
    }

    /**
     *
     * Get home URL for Lev site.
     *
     * @param  string|null $lang     Optional language code for multilingual links.
     * @param  bool|null   $absolute If true, return absolute url, if false, return relative url. Otherwise return default.
     * @return string
    ...
    public function addPage(PageInterface $page, $route = null): void
    {
        ...
        $this->lev->fireEvent('onPageProcessed', new Event(['page' => $page]));
    }
    ...
    public function getCollection(array $params = [], array $context = [])
    {
        ...
        if ($context['event']) {
            $this->lev->fireEvent('onCollectionProcessed', new Event(['collection' => $collection, 'context' => $context]));
        }
    ...
    protected function evaluate($value, PageInterface $self = null)
    {
        ...
        switch ($scope) {
            ...
            case '@taxonomy':
                ...
                $taxonomy_map = Lev::instance()['taxonomy'];
        ...
        if ($site_route) {
            ...
            foreach (array_reverse($site_routes, true) as $pattern => $replace) {
                ...
                try {
                    ...
                    $this->lev['log']->error('site.routes: ' . $pattern . '-> ' . $e->getMessage());
                }
            }
        }
    ...
    public function dispatch($route, $all = false, $redirect = true)
    {
        ...
        if ($page) {
            ...
            if ($redirect) {
                if ($page->redirect()) {
                    ...
                    $this->lev->redirectLangSafe($page->redirect());
                }
                ...
                if (!$routable) {
                    ...
                    if ($child !== null) {
                        ...
                        $this->lev->redirectLangSafe($child->route());
                    }
                }
            }
        ...
        /** @var Uri $uri */
        $uri = $this->lev['uri'];
        /** @var \Lev\Framework\Uri\Uri $source_url */
            ...
            foreach ((array)$site_redirects as $pattern => $replace) {
                ...
                try {
                    ...
                    if ($found && $found !== $source_url) {
                        $this->lev->redirectLangSafe($found);
                    }
                } catch (ErrorException $e) {
                    $this->lev['log']->error('site.redirects: ' . $pattern . '-> ' . $e->getMessage());
                }
            }
        }
    ...
    public function blueprints($type)
    {
        ...
        if (empty($blueprint->initialized)) {
            ...
            $this->lev->fireEvent('onBlueprintCreated', new Event(['blueprint' => $blueprint, 'type' => $type]));
        }
    ...
    public static function getTypes()
    {
        if (null === self::$types) {
            ...
            $scanBlueprintsAndTemplates = static function (Types $types) use ($lev) {
                // Scan blueprints
                $event = new Event();
                $event->types = $types;
                $lev->fireEvent('onGetPageBlueprints', $event);
                ...
                $event = new Event();
                $event->types = $types;
                $lev->fireEvent('onGetPageTemplates', $event);

                $types->scanTemplates('theme://templates/');
            };

            if ($lev['config']->get('system.cache.enabled')) {
                /** @var Cache $cache */
                $cache = $lev['cache'];
    ...
    {
        if (null === $type && isset(Lev::instance()['admin'])) {
            /** @var Admin $admin */
    ...
    protected function initFlexPages(): void
    {
        ...
        $dispatcher->addListener(
            ...
            static function (Event $event) use ($directory) {
                ...
                if ($location !== 'pages' || isset($lev['flex_objects'])) {
                    return;
                }
    ...
    protected function buildFlexPages(FlexDirectory $directory): void
    {
        ...
        if ($this->fire_events) {
            $this->lev->fireEvent('onBuildPagesInitialized');
        }
        ...
        foreach ($this->children as $path => $list) {
            ...
            if ($this->fire_events) {
                $this->lev->fireEvent('onFolderProcessed', new Event(['page' => $page]));
            }
            // Sort the children.
    ...
    protected function buildRegularPages(): void
    {
        ...
        if ($config->get('system.cache.enabled')) {
            ...
            $this->lev['debugger']->addMessage('Page cache missed, rebuilding pages..');
        } else {
            $this->lev['debugger']->addMessage('Page cache disabled, rebuilding pages..');
        }
    ...
    public function resetPages($pages_dir): void
    {
        ...
        // cache if needed
        if ($this->lev['config']->get('system.cache.enabled')) {
            /** @var Cache $cache */
            $cache = $this->lev['cache'];
            /** @var Taxonomy $taxonomy */
            $taxonomy = $this->lev['taxonomy'];
    ...
    protected function buildSort($path, array $pages, $order_by = 'default', $manual = null, $sort_flags = null): void
    {
        ...
        if ($order_by === 'random') {
            ...
            if (extension_loaded('intl') && $this->lev['config']->get('system.intl_enabled')) {
