...
use Lev\Common\Markdown\Parsedown;
use Lev\Common\Markdown\ParsedownExtra;
use Lev\Common\Page\Interfaces\PageCollectionInterface;
use Lev\Common\Page\Interfaces\PageInterface;
use Lev\Common\Media\Traits\MediaTrait;
use Lev\Common\Page\Markdown\Excerpts;
use Lev\Common\Page\Traits\PageFormTrait;
...
class Page implements PageInterface
{
    ...
    protected function processFrontmatter()
    {
        ...
        if (Utils::contains(json_encode(array_values($process_fields)), '{{')) {
            $ignored_fields = [];
            foreach ((array)Lev::instance()['config']->get('system.pages.frontmatter.ignore_fields') as $field) {
                if (isset($process_fields[$field])) {
                    $ignored_fields[$field] = $process_fields[$field];
                    unset($process_fields[$field]);
                }
            }
            $text_header = Lev::instance()['twig']->processString(json_encode($process_fields, JSON_UNESCAPED_UNICODE), ['page' => $this]);
    ...
    public function translatedLanguages($onlyPublished = false)
    {
        ...
        foreach ($languages as $languageCode) {
            ...
            $path = $this->path . '/' . $this->folder . '/' . $name . $languageExtension;
            ...
            if ($file) {
                try {
                    ...
                    if (!Utils::isAdminPlugin()) {
                        ...
                        if (Lev::instance()['config']->get('system.pages.frontmatter.process_twig') === true) {
                            $this->processFrontmatter();
                        }
                    }
                } catch (Exception $e) {
                    $file->raw(Lev::instance()['language']->translate([
                        'LEV.FRONTMATTER_ERROR_PAGE',
                        $this->slug(),
                        $file->filename(),
                        $e->getMessage(),
                        $file->raw()
    ...
    public function httpHeaders()
    {
        ...
        // Ask Lev to calculate ETag from the final content.
        if ($this->eTag()) {
            $headers['ETag'] = '1';
        }

        // Set Vary: Accept-Encoding header
        if ($lev['config']->get('system.pages.vary_accept_encoding', false)) {
    ...
    {
        $config = (array)Lev::instance()['config']->get('site.summary');
        ...
        if ($this->content === null) {
            ...
            if ($never_cache_twig) {
                if ($this->content === false || $cache_enable === false) {
                    ...
                    Lev::instance()->fireEvent('onPageContentRaw', new Event(['page' => $this]));
                    ...
                    Lev::instance()->fireEvent('onPageContentProcessed', new Event(['page' => $this]));
            ...
            Lev::instance()->fireEvent('onPageContent', new Event(['page' => $this]));
        }
    ...
    protected function processMarkdown(bool $keepTwig = false)
    {
        ...
        if (!isset($markdownDefaults['extra']) && (isset($this->markdown_extra) || $config->get('system.pages.markdown_extra') !== null)) {
            user_error('Configuration option \'system.pages.markdown_extra\' is deprecated since Lev 1.5, use \'system.pages.markdown.extra\' instead', E_USER_DEPRECATED);
    ...
    public function move(PageInterface $parent)
    {
        ...
        if ($parent->route()) {
            ...
            $this->route(Lev::instance()['pages']->root()->route() . '/' . $this->slug());
        }
    ...
    public function blueprints()
    {
        ...
        $edit_mode = isset($lev['admin']) ? $lev['config']->get('plugins.admin.edit_mode') : null;
    ...
    public function urlExtension()
    {
        ...
        if (null === $this->url_extension) {
            $this->url_extension = Lev::instance()['config']->get('system.pages.append_url_extension', '');
        }
    ...
    public function expires($var = null)
    {
        ...
        return $this->expires ?? Lev::instance()['config']->get('system.pages.expires');
    }
    ...
    public function cacheControl($var = null)
    {
        ...
        return $this->cache_control ?? Lev::instance()['config']->get('system.pages.cache_control');
    }
    ...
    public function id($var = null)
    {
        ...
        if ($var !== null) {
            ...
            $active_lang = Lev::instance()['language']->getLanguage() ?: '';
            $id = $active_lang . $var;
            $this->id = $id;
        }
    ...
    public function eTag($var = null): bool
    {
        ...
        if (!isset($this->etag)) {
            $this->etag = (bool)Lev::instance()['config']->get('system.pages.etag');
        }
    ...
    public function lastModified($var = null)
    {
        ...
        if (!isset($this->last_modified)) {
            $this->last_modified = (bool)Lev::instance()['config']->get('system.pages.last_modified');
        }
    ...
    {
        return str_replace(LEV_SITE_DIR . '/', '', $this->filePath());
    }
    ...
    {
        user_error(__METHOD__ . '() is deprecated since Lev 1.7, use ->isModule() or ->modularTwig() method instead', E_USER_DEPRECATED);
    ...
    {
        $uri_path = rtrim(urldecode(Lev::instance()['uri']->path()), '/') ?: '/';
        $routes = Lev::instance()['pages']->routes();
    ...
    {
        $home = Lev::instance()['config']->get('system.home.alias');
    ...
    protected function setPublishState()
    {
        ...
        if (Lev::instance()['config']->get('system.pages.publish_dates') && !isset($this->header->published)) {
    ...
    {
        $case_insensitive = Lev::instance()['config']->get('system.force_lowercase_urls');
