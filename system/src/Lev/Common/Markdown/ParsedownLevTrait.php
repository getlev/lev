...
trait ParsedownLevTrait
{
    ...
    /**
     * Initialization function to setup key variables needed by the MarkdownLevLinkTrait
     *
    ...
    protected function init($excerpts = null, $defaults = null)
    {
        if (!$excerpts || $excerpts instanceof PageInterface) {
            ...
            user_error(__CLASS__ . '::' . __FUNCTION__ . '($page, $defaults) is deprecated since Lev 1.6.10, use ->init(new ' . Excerpts::class . '($page, [\'markdown\' => $defaults])) instead.', E_USER_DEPRECATED);
        } else {
