...
/**
 * @package    Lev\Grav\Common\Language
 *
 * @copyright  Copyright (c) 2015 - 2022 Levitation, LLC. All rights reserved.
 * @license    MIT License; see LICENSE file for details.
 */

namespace Lev\Common\Language;
...
class Language
{
    ...
    public function setActiveFromUri($uri)
    {
        ...
        if ($this->enabled()) {
            ...
            if (preg_match($regex, $uri, $matches)) {
                ...
                if (isset($this->lev['session']) && $this->lev['session']->isStarted()
                    && $this->config->get('system.languages.session_store_active', true)
                    && $this->lev['session']->active_language != $this->active
                ) {
                    $this->lev['session']->active_language = $this->active;
                }
            } else {
                ...
                if (isset($this->lev['session']) && $this->lev['session']->isStarted() &&
                    $this->config->get('system.languages.session_store_active', true)) {
                    $this->setActive($this->lev['session']->active_language ?: null);
                }
     ...
     *
     * ```
     * $this->lev['language']->setActive('it');
     * $this->lev['language']->resetFallbackPageExtensions();
     * $this->lev['pages']->init();
     * ```
     *
     * @return void
    ...
    public function getFallbackLanguages(string $languageCode = null, bool $includeDefault = false)
    {
        ...
        if (!isset($this->fallback_languages[$key])) {
            ...
            if (null === $fallback && $this->config->get('system.languages.pages_fallback_only', false)) {
                user_error('Configuration option `system.languages.pages_fallback_only` is deprecated since Lev 1.7, use `system.languages.content_fallback` instead', E_USER_DEPRECATED);
    ...
    public function translateArray($key, $index, $languages = null, $html_out = false)
    {
        ...
        foreach ((array)$languages as $lang) {
            $translation_array = (array)Lev::instance()['languages']->get($lang . '.' . $key, null);
    ...
    public function getTranslation($lang, $key, $array_support = false)
    {
        ...
        $translation = Lev::instance()['languages']->get($lang . '.' . $key, null);
    ...
    {
        user_error(__CLASS__ . '::' . __FUNCTION__ . '() is deprecated since Lev 1.6, no longer used', E_USER_DEPRECATED);
