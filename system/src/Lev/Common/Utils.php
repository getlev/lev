...
use Lev\Common\Helpers\Truncator;
...
abstract class Utils
{
    ...
    /**
     * Simple helper method to make getting a Lev URL easier
     *
    ...
    public static function url($input, $domain = false, $fail_gracefully = false)
    {
        ...
        if (static::contains((string)$input, '://')) {
            // Url contains a scheme (https:// , app:// etc).
            ...
            if (is_array($parts)) {
                ...
                if ($scheme && !$port) {
                    // If URL has a scheme, we need to check if it's one of Lev streams.
        ...
        $domain = $domain ?: $lev['config']->get('system.absolute_urls', false);
    ...
    public static function fullPath($path)
    {
        ...
        } elseif (!static::startsWith($path, LEV_SITE_DIR)) {
            $base_url = Lev::instance()['base_url'];
            $path = LEV_SITE_DIR . '/' . ltrim(static::replaceFirstOccurrence($base_url, '', $path), '/');
        }
    ...
    /**
     * Return the Lev date formats allowed
     *
     * @return array
     */
    public static function dateFormats()
    {
        ...
        $default_format = Lev::instance()['config']->get('system.pages.dateformat.default');
        if ($default_format) {
        ...
        if (file_exists($file)) {
            ...
            if (isset($_SERVER['HTTP_RANGE'])) {
                ...
                if ($lev['config']->get('system.cache.enabled')) {
                    $expires = $options['expires'] ?? $lev['config']->get('system.pages.expires');
                    if ($expires > 0) {
    ...
    public static function getMimeByExtension($extension, $default = 'application/octet-stream')
    {
        ...
        $media_types = Lev::instance()['config']->get('media.types');
    ...
    public static function getExtensionsByMime($mime)
    {
        ...
        $media_types = (array)Lev::instance()['config']->get('media.types');
    ...
    {
        $dangerous_extensions = Lev::instance()['config']->get('security.uploads_dangerous_extensions', []);
    ...
    {
        $languages_enabled = Lev::instance()['config']->get('system.languages.supported', []);
    ...
    {
        user_error(__CLASS__ . '::' . __FUNCTION__ . '() is deprecated since Lev 1.5, use ->getDotNotation() method instead', E_USER_DEPRECATED);
    ...
    private static function generateNonceString($action, $previousTick = false)
    {
        ...
        $username = isset($lev['user']) ? $lev['user']->username : '';
        $token = session_id();
        $i = self::nonceTick();
        ...
        return ($i . '|' . $action . '|' . $username . '|' . $token . '|' . $lev['config']->get('security.salt'));
    }
    ...
    {
        return isset(Lev::instance()['admin']);
    }
        ...
        switch ($matches[0]) {
            ...
            case 'page':
                if ($matches[1] === '') {
                    ...
                    $key = trim($route === '/' ? $lev['config']->get('system.home.alias') : $route, '/');
            ...
            case 'theme':
                if ($matches[1] === '') {
                    $route = '/' . $matches[2];
                    $theme = $lev['locator']->findResource('theme://', false);
                    if (false !== $theme) {
    ...
    {
        $lev = Lev::instance();
        $page = $page ?? $lev['page'] ?? null;
        $defaults = [
            'markdown' => $lev['config']->get('system.pages.markdown', []),
            'images' => $lev['config']->get('system.images', [])
        ];
    ...
    {
        $types = Lev::instance()['config']->get('system.pages.types', $defaults);
        if (!is_array($types)) {
            return [];
        }
