...
use Lev\Framework\File\YamlFile;
...
class InitializeProcessor extends ProcessorBase
{
     ...
     * @return void
     */
    public static function initializeCli(Lev $lev)
    {
        if (!static::$cli_initialized) {
            static::$cli_initialized = true;

            $instance = new static($lev);
            $instance->processCli();
        }
    }
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        // Lev may return redirect response right away.
    ...
    protected function initializeConfig(): Config
    {
        ...
        if (defined('LEV_SCHEMA') && $config->get('versions') === null) {
            $filename = LEV_SITE_DIR . '/' . LEV_APP_PATH . '/' . 'config/versions.yaml';
            if (!is_file($filename)) {
                $versions = [
                    'core' => [
                        'lev' => [
    ...
    protected function initializeLogger(Config $config): Logger
    {
        ...
        if ($config->get('system.log.handler', 'file') === 'syslog') {
            ...
            $logHandler = new SyslogHandler('lev', $facility);
...
//                $baseUri = '';
//            }
            $requestTime = $server['REQUEST_TIME_FLOAT'] ?? LEV_REQUEST_TIME;
    ...
    protected function initializePages(Config $config): Pages
    {
        ...
        /** @var Pages $pages */
        $pages = $lev['pages'];
        // Upgrading from older Lev versions won't work without checking if the method exists.
