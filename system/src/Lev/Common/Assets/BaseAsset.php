...
use Lev\Common\Assets\Traits\AssetUtilsTrait;
use Lev\Common\Config\Config;
use Lev\Common\Lev;
use Lev\Common\Uri;
use Lev\Common\Utils;
use Lev\Framework\Object\PropertyObject;
...
abstract class BaseAsset extends PropertyObject
{
    ...
    public static function integrityHash($input)
    {
        ...
        $assetsConfig = $lev['config']->get('system.assets');
        ...
        if (!self::isRemoteLink($input) && !empty($assetsConfig['enable_asset_sri']) && $assetsConfig['enable_asset_sri']) {
            ...
            $asset = File::instance(LEV_SITE_DIR . $input);
...
//    {
//        $file = LEV_SITE_DIR . $asset;
//        if (Lev::instance()['locator']->isStream($asset)) {
...
//    }
    ...
    /**
     *
     * Build local links including lev asset shortcodes
     *
     * @param  string $asset    the asset string reference
     *
    ...
    {
        if ($asset) {
            return $this->base_url . ltrim(Utils::replaceFirstOccurrence(LEV_SITE_DIR, '', $asset), '/');
        }
        return false;
    }
