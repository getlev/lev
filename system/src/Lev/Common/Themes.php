...
class Themes extends Iterator
{
    ...
    public function initTheme()
    {
        if ($this->inited === false) {
            ...
            if (is_dir('theme://blueprints/pages')) {
                ...
                $locator->addPath('blueprints', '', ['theme://blueprints'], ['app', 'blueprints']);
            }
            ...
            $this->lev['theme'] = $instance;

            $this->lev->fireEvent('onThemeInitialized');
    ...
    public function get($name)
    {
        ...
        $path = $this->lev['locator']->findResource($thumb, false);

        if ($path) {
            $blueprint->set('thumbnail', $this->lev['base_url'] . '/' . $path);
        }
    ...
    public function load()
    {
        ...
        if ($file) {
            // Local variables available in the file: $lev, $config, $name, $file
            $class = include $file;
            if (!\is_object($class) || !is_subclass_of($class, Theme::class, true)) {
                $class = null;
            }
        } elseif (!$locator('theme://') && !defined('LEV_CLI')) {
        ...
        if (null === $class) {
            $themeClassFormat = [
                'Lev\\Theme\\' . Inflector::camelize($name),
                'Lev\\Theme\\' . ucfirst($name)
            ];

            foreach ($themeClassFormat as $themeClass) {
                if (is_subclass_of($themeClass, Theme::class, true)) {
                    $class = new $themeClass($lev, $config, $name);
                    break;
                }
            }
        }
        ...
        if (null === $class) {
            $class = new Theme($lev, $config, $name);
        }
    ...
    protected function loadLanguages(Config $config)
    {
        ...
        if ($config->get('system.languages.translations', true)) {
            ...
            foreach ($language_files as $language_file) {
                ...
                $this->lev['languages']->mergeRecursive($language);
            }
            ...
            foreach ($languages_folders as $languages_folder) {
                ...
                $this->lev['languages']->mergeRecursive($languages);
            }
        }
    }
    ...
    protected function autoloadTheme($class)
    {
        ...
        if (false !== strpos($class, $prefix)) {
            ...
            $path = $this->lev['inflector']->hyphenize($class);
            $file = $locator("themes://{$path}/theme.php") ?: $locator("themes://{$path}/{$path}.php");
