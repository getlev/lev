...
/**
 * @package    Lev\Grav\Common\Scheduler
 * @author     Originally based on peppeocchi/php-cron-scheduler modified for Lev integration
 * @copyright  Copyright (c) 2015 - 2022 Levitation, LLC. All rights reserved.
 * @license    MIT License; see LICENSE file for details.
 */

namespace Lev\Common\Scheduler;
...
class Scheduler
{
    ...
    {
        $config = Lev::instance()['config']->get('scheduler.defaults', []);
        $this->config = $config;

        $this->status_path = Lev::instance()['locator']->findResource('user-data://scheduler', true, true);
    ...
    {
        $this->saved_jobs = [];
        $saved_jobs = (array) Lev::instance()['config']->get('scheduler.custom_jobs', []);

        foreach ($saved_jobs as $id => $j) {
            $args = $j['args'] ?? [];
            $id = Lev::instance()['inflector']->hyphenize($id);
    ...
    public function getSchedulerCommand($php = null)
    {
        ...
        $command = 'cd ' . str_replace(' ', '\ ', LEV_SITE_DIR) . ';' . $php . ' bin/lev scheduler';
