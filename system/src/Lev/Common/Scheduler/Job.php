...
class Job
{
    ...
    {
        if (is_string($id)) {
            $this->id = Lev::instance()['inflector']->hyphenize($id);
        } else {
        ...
        $this->args = $args;
        // Set enabled state
        $status = Lev::instance()['config']->get('scheduler.status');
    ...
    private function emailOutput()
    {
        ...
        if (is_callable('Lev\Plugin\Email\Utils::sendEmail')) {
            $subject ='Lev Scheduled Job [' . $this->getId() . ']';
            ...
            \Lev\Plugin\Email\Utils::sendEmail($subject, $content, $to);
        }
