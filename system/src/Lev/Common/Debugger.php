...
use Lev\Common\Processors\ProcessorInterface;
use Lev\Common\Twig\TwigClockworkDataSource;
...
class Debugger
{
    ...
    public function __construct()
    {
        ...
        if (!defined('LEV_REQUEST_TIME')) {
            define('LEV_REQUEST_TIME', $this->currentTime);
        }

        $this->requestTime = $_SERVER['REQUEST_TIME_FLOAT'] ?? LEV_REQUEST_TIME;
    ...
    public function init()
    {
        ...
        if ($this->enabled) {
            ...
            if ($clockwork) {
                ...
                if ($this->requestTime !== LEV_REQUEST_TIME) {
                    $event = $timeline->event('Server');
                    $event->finalize($this->requestTime, LEV_REQUEST_TIME);
                }
                if ($this->currentTime !== LEV_REQUEST_TIME) {
                    $event = $timeline->event('Loading');
                    $event->finalize(LEV_REQUEST_TIME, $this->currentTime);
                }
            ...
            if ($debugbar) {
                ...
                if ($this->requestTime !== LEV_REQUEST_TIME) {
                    $debugbar['time']->addMeasure('Server', $debugbar['time']->getRequestStartTime(), LEV_REQUEST_TIME);
                }
                if ($this->currentTime !== LEV_REQUEST_TIME) {
                    $debugbar['time']->addMeasure('Loading', LEV_REQUEST_TIME, $this->currentTime);
                }
                $debugbar['time']->addMeasure('Site Setup', $this->currentTime, microtime(true));
            }

            $this->addMessage('Lev v' . LEV_GVERSION . ' - PHP ' . PHP_VERSION);
            $this->config->debug();
    ...
    public function logRequest(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
    {
        ...
        $lev = Lev::instance();
        $basePath = $this->lev['base_url_relative'] . $lev['pages']->base();
        if ($basePath) {
    ...
    /**
     * Add the debugger assets to the Lev Assets
     *
     * @return $this
     */
    ...
    protected function buildProfilerTimings(array $timings): array
    {
        ...
        foreach ($timings as $key => $timing) {
            ...
            if ($context === 'Lev\Framework\RequestHandler\RequestHandler::handle()') {
                continue;
            }

            // Do not profile library calls.
            if (strpos($context, 'Lev\\') !== 0) {
                continue;
            }
    ...
    protected function getDepracatedMessage($deprecated)
    {
        ...
        if (isset($deprecated['trace'])) {
            foreach ($deprecated['trace'] as $current) {
                ...
                if (isset($current['file'])) {
                    $current['file'] = str_replace(LEV_SITE_DIR . '/', '', $current['file']);
                }
