 ...
 * @package Lev\Common
 */
class Session extends \Lev\Framework\Session\Session
{
    /** @var bool */
    protected $autoStart = false;

    /**
     * @return \Lev\Framework\Session\Session
     * @deprecated 1.5 Use ->getInstance() method instead.
     */
    public static function instance()
    {
        user_error(__CLASS__ . '::' . __FUNCTION__ . '() is deprecated since Lev 1.5, use ->getInstance() method instead', E_USER_DEPRECATED);
    ...
    public function all()
    {
        user_error(__CLASS__ . '::' . __FUNCTION__ . '() is deprecated since Lev 1.5, use ->getAll() method instead', E_USER_DEPRECATED);
    ...
    {
        user_error(__CLASS__ . '::' . __FUNCTION__ . '() is deprecated since Lev 1.5, use ->isStarted() method instead', E_USER_DEPRECATED);
    ...
    public function getFlashObject($name)
    {
        ...
        if ($name === 'files-upload') {
            ...
            if (null === $object && isset($lev['forms'])) {
                user_error(
                    __CLASS__ . '::' . __FUNCTION__ . '(\'files-upload\') is deprecated since Lev 1.6, use $form->getFlash()->getLegacyFiles() instead',
                    E_USER_DEPRECATED
                );
                ...
                $uri = $lev['uri'];
                /** @var Forms|null $form */
                $form = $lev['forms']->getActiveForm(); // @phpstan-ignore-line (form plugin)
