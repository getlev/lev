...
use Lev\Framework\File\JsonFile;
 ...
 */
class InstallCommand extends LevCommand
{
    /** @var array */
    protected $config;
    /** @var string */
    ...
    protected function configure(): void
    {
        $this
            ...
            ->setDescription('Installs the dependencies needed by Lev. Optionally can create symbolic links')
            ->setHelp('The <info>install</info> command installs the dependencies needed by Lev. Optionally can create symbolic links');
    }
    ...
    protected function serve(): int
    {
        ...
        $this->destination = $input->getArgument('destination') ?: LEV_SITE_DIR;

        // fix trailing slash
        $this->destination = rtrim($this->destination, '/') . '/';
        $this->user_path = $this->destination . LEV_APP_PATH . '/';
        ...
        if (file_exists($this->user_path . $dependencies_file)) {
            ...
            if ($input->getArgument('destination')) {
                $io->writeln('<yellow>HINT</yellow> <info>Are you trying to install a plugin or a theme? Make sure you use <cyan>bin/gpm install <something></cyan>, not <cyan>bin/lev install</cyan>. This command is only used to install Lev skeletons.');
            } else {
                $io->writeln('<yellow>HINT</yellow> <info>Are you trying to install Lev? Lev is already installed. You need to run this command only if you download a skeleton from GitHub directly.');
            }
        ...
        if (!$symlink) {
            ...
            $io->writeln($this->composerUpdate(LEV_SITE_DIR, 'install'));
    ...
    private function readHebe(string $folder): ?array
    {
        ...
        $hebe = $file->load();
        $paths = $hebe['platforms']['lev']['nodes'] ?? null;
