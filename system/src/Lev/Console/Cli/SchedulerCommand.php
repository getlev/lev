 ...
 */
class SchedulerCommand extends LevCommand
{
    /**
     * @return void
     */
    protected function configure(): void
    {
        $this
            ...
            ->setDescription('Run the Lev Scheduler.  Best when integrated with system cron')
    ...
    protected function serve(): int
    {
        ...
        $scheduler = $lev['scheduler'];
        $lev->fireEvent('onSchedulerInitialized', new Event(['scheduler' => $scheduler]));
        ...
        if ($input->getOption('jobs')) {
            ...
            $io->newLine();
            $io->note('For error details run "bin/lev scheduler -d"');
            $io->newLine();
            ...
            if ($scheduler->isCrontabSetup()) {
                $io->success('All Ready! You have already set up Lev\'s Scheduler in your crontab. You can validate this by running "crontab -l" to list your current crontab entries.');
                $verb = 'reinstall';
            } else {
                $user = $scheduler->whoami();
                $error = 1;
                $io->error('Can\'t find a crontab for ' . $user . '. You need to set up Lev\'s Scheduler in your crontab');
            }
            if (!Utils::isWindows()) {
