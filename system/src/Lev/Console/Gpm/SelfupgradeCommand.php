...
class SelfupgradeCommand extends GpmCommand
{
    ...
    protected function configure(): void
    {
        $this
            ...
            ->setDescription('Detects and performs an update of Lev itself when available')
            ->setHelp('The <info>update</info> command updates Lev itself when a new version is available');
    }
    ...
    protected function serve(): int
    {
        ...
        $update = $this->upgrader->getAssets()['lev-update'];
        ...
        if (!$this->upgrader->meetsRequirements()) {
            ...
            $io->writeln('   Lev has increased the minimum PHP requirement.');
        ...
        if (!$this->overwrite && !$this->upgrader->isUpgradable()) {
            $io->writeln("You are already running the latest version of <green>Lev v{$local}</green>");
            $io->writeln("which was released on {$release}");

            $config = Lev::instance()['config'];
            $schema = $config->get('versions.core.lev.schema');
            if ($schema !== LEV_SCHEMA && version_compare($schema, LEV_SCHEMA, '<')) {
                $io->newLine();
        ...
        if (Installer::IS_LINK === Installer::lastErrorCode()) {
            $io->writeln('<red>ATTENTION:</red> Lev is symlinked, cannot upgrade, aborting...');
            $io->newLine();
            $io->writeln("You are currently running a symbolically linked Lev v{$local}. Latest available is v{$remote}.");
        ...
        $io->writeln("Lev v<cyan>{$remote}</cyan> is now available [release date: {$release}].");
        $io->writeln('You are currently using v<cyan>' . LEV_GVERSION . '</cyan>.');
        ...
        if (!$this->all_yes) {
            ...
            if ($answer) {
                $changelog = $this->upgrader->getChangelog(LEV_GVERSION);
    ...
    private function download(array $package): string
    {
        ...
        $this->tmp = $tmp_dir . '/lev-update-' . uniqid('', false);
        $options = [
        ...
        file_put_contents($this->tmp . '/' . $package['name'], $output);

        return $this->tmp . '/' . $package['name'];
    }
     ...
     * @return void
     */
    private function upgradeLev(string $zip): void
    {
        try {
