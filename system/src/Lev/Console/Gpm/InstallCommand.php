...
use Lev\Common\GPM\Remote\Package;
use Lev\Common\Lev;
...
class InstallCommand extends GpmCommand
{
    ...
    protected function serve(): int
    {
        ...
        if (count($this->data['not_found'])) {
            $io->writeln('These packages were not found on Lev: <red>' . implode(
                '</red>, <red>',
        ...
        if (null !== $this->local_config) {
            // Symlinks available, ask if Lev should use them
            $this->use_symlinks = false;
            $question = new ConfirmationQuestion('Should Lev use the symlinks if available? [y|N] ', false);
        ...
        foreach ($this->data as $data) {
            foreach ($data as $package_name => $package) {
                if (array_key_exists($package_name, $dependencies)) {
                    ...
                    $is_valid_destination = Installer::isValidDestination($this->destination . '/' . $package->install_path);
    ...
    {
        $demo_dir = $this->destination . '/' . $package->install_path . '/' . '_demo';
    ...
    private function installDemoContent(Package $package): void
    {
        ...
        if (file_exists($demo_dir)) {
            $dest_dir = $this->destination . '/' . 'user';
            ...
            if (file_exists($demo_dir . '/' . 'pages')) {
                ...
                if (file_exists($dest_dir)) {
                    if (rename($pages_dir, $dest_dir . '/' . $pages_backup)) {
    ...
    private function processSymlink(Package $package): void
    {
        ...
        $to = $this->destination . '/' . $package->install_path;
    ...
    private function downloadPackage(Package $package, string $license = null)
    {
        ...
        file_put_contents($this->tmp . '/' . $filename, $output);

        return $this->tmp . '/' . $filename;
    }
    ...
    private function checkDestination(Package $package): bool
    {
        ...
        Installer::isValidDestination($this->destination . '/' . $package->install_path);
        ...
        if (Installer::lastErrorCode() === Installer::IS_LINK) {
            ...
            unlink($this->destination . '/' . $package->install_path);
        }
