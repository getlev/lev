...
trait ConsoleTrait
{
     ...
     * @return void
     */
    final protected function setupLev(): void
    {
        try {
        ...
        $lev = Lev::instance();
        $lev['config']->set('system.cache.cli_compatibility', true);
    }
     ...
     * @return $this
     */
    final protected function initializeLev()
    {
        InitializeProcessor::initializeCli(Lev::instance());
    ...
    final protected function initializePages()
    {
        if (!$this->pages_initialized) {
            ...
            // Initialize assets.
            $lev['assets']->init();
            $lev->fireEvent('onAssetsInitialized');
            ...
            $pages = $lev['pages'];
            $pages->init();
            $lev->fireEvent('onPagesInitialized', new Event(['pages' => $pages]));
        }
     ...
     * @return void
     */
    public function isLevInstance($path)
    {
        $io = $this->getIO();
        ...
        if (!file_exists($path . '/' . 'index.php') || !file_exists($path . '/' . '.dependencies') || !file_exists($path . '/' . 'system' . '/' . 'config' . '/' . 'system.yaml')) {
            $io->writeln('');
            $io->writeln('<red>ERROR</red>: Destination chosen to install does not appear to be a Lev instance:');
            $io->writeln("       <white>$path</white>");
            $io->writeln('');
            exit;
        }
    ...
    public function loadLocalConfig()
    {
        ...
        $local_config_file = $home_folder . '/.lev/config';
