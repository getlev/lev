...
use Lev\Framework\Cache\Adapter\DoctrineCache;
use Lev\Framework\Cache\Adapter\MemoryCache;
...
use Lev\Framework\Flex\Interfaces\FlexStorageInterface;
use Lev\Framework\Flex\Storage\SimpleStorage;
...
use Lev\Common\Flex\Types\Generic\GenericObject;
use Lev\Common\Flex\Types\Generic\GenericCollection;
use Lev\Common\Flex\Types\Generic\GenericIndex;
...
class FlexDirectory implements FlexDirectoryInterface
{
    ...
    public function loadDirectoryConfig(string $name): array
    {
        ...
        if (str_starts_with($uri, 'config://')) {
            ...
            return (array)$lev['config']->get($path);
        }
        ...
        if (null === $cache) {
            try {
                ...
                if (empty($config['enabled'])) {
                    ...
                    $cache = new DoctrineCache($levCache->getCacheDriver(), 'flex-objects-' . $this->getFlexType() . $key, $lifetime);
                }
            } catch (Exception $e) {
    ...
    protected function getBlueprintInternal(string $type_view = '', string $context = '')
    {
        if (!isset($this->blueprints[$type_view])) {
            ...
            if ($blueprint->get('type') === 'flex-objects' && isset(Lev::instance()['admin'])) {
                $blueprintBase = (new Blueprint('plugins://flex-objects/blueprints/flex-objects.yaml'))->load();
    ...
    protected function dynamicDataField(array &$field, $property, array $call)
    {
        ...
        $object = $call['object'];
        if ($function === '\Lev\Common\Page\Pages::pageTypes') {
    ...
    {
        user_error(__CLASS__ . '::' . __FUNCTION__ . '() is deprecated since Lev 1.6, use ->getFlexType() method instead', E_USER_DEPRECATED);
