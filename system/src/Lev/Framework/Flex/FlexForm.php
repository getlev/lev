...
use Lev\Framework\Flex\Interfaces\FlexObjectFormInterface;
...
class FlexForm implements FlexObjectFormInterface, JsonSerializable
{
    ...
    public function getFileUploadAjaxRoute(): ?Route
    {
        ...
        if (!method_exists($object, 'route')) {
            /** @var Route $route */
            $route = Lev::instance()['route'];

            return $route->withExtension('json')->withLevParam('task', 'media.upload');
        }
    ...
    public function getFileDeleteAjaxRoute($field = null, $filename = null): ?Route
    {
        ...
        if (!method_exists($object, 'route')) {
            ...
            return $route->withExtension('json')->withLevParam('task', 'media.delete');
        }
