...
use Lev\Framework\Cache\CacheInterface;
...
use Lev\Framework\Flex\Traits\FlexAuthorizeTrait;
use Lev\Framework\Flex\Traits\FlexRelatedDirectoryTrait;
...
class FlexObject implements FlexObjectInterface, FlexAuthorizeInterface
{
    ...
    public function __construct(array $elements, $key, FlexDirectory $directory, bool $validate = false)
    {
        if (get_class($this) === __CLASS__) {
            user_error('Using ' . __CLASS__ . ' directly is deprecated since Lev 1.7, use \Lev\Common\Flex\Types\Generic\GenericObject or your own class instead', E_USER_DEPRECATED);
        }
    ...
    {
        user_error(__METHOD__ . '() is deprecated since Lev 1.6, use ->getFlexType() method instead', E_USER_DEPRECATED);
    ...
    {
        user_error(__METHOD__ . '() is deprecated since Lev 1.6, use ->getFormValue() method instead', E_USER_DEPRECATED);
     ...
     * @return $this
     * @deprecated 1.7 Moved to \Lev\Common\Flex\Traits\FlexObjectTrait
     */
    public function triggerEvent(string $name, $event = null)
    {
        user_error(__METHOD__ . '() is deprecated since Lev 1.7, moved to \Lev\Common\Flex\Traits\FlexObjectTrait', E_USER_DEPRECATED);
        ...
        if ($event instanceof Event) {
            $lev->fireEvent($name, $event);
        } else {
            $lev->dispatchEvent($event);
        }
    ...
    {
        user_error(__METHOD__ . '() is deprecated since Lev 1.7, use ->setMetaData() method instead', E_USER_DEPRECATED);
    ...
    {
        user_error(__METHOD__ . '() is deprecated since Lev 1.7, use ->getMetaData() method instead', E_USER_DEPRECATED);
     ...
     * @throws LoaderError
     * @throws SyntaxError
     * @deprecated 1.7 Moved to \Lev\Common\Flex\Traits\LevTrait
     */
    protected function getTemplate($layout)
    {
        user_error(__METHOD__ . '() is deprecated since Lev 1.7, moved to \Lev\Common\Flex\Traits\LevTrait', E_USER_DEPRECATED);
    ...
    protected function getAuthorizeScope(): string
    {
        ...
        return isset(Lev::instance()['admin']) ? 'admin' : 'site';
    }
}
