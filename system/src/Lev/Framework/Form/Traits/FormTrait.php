...
namespace Lev\Framework\Form\Traits;
...
use Lev\Framework\Compat\Serializable;
...
trait FormTrait
{
    ...
    public function getFlash()
    {
        if (null === $this->flash) {
            ...
            $this->flash->setUrl($lev['uri']->url)->setUser($lev['user'] ?? null);
        }
    ...
    public function render(string $layout = null, array $context = [])
    {
        ...
        $output = $this->getTemplate($layout)->render(
            ['lev' => $lev, 'config' => $lev['config'], 'block' => $block, 'form' => $this, 'layout' => $layout] + $context
        );
    ...
    protected function validateUpload(UploadedFileInterface $file): void
    {
        ...
        if ($filename && !Utils::checkFilename($filename)) {
            ...
            throw new RuntimeException(
                sprintf($lev['language']->translate('PLUGIN_FORM.FILEUPLOAD_UNABLE_TO_UPLOAD', null, true), $filename, 'Bad filename')
            );
        }
