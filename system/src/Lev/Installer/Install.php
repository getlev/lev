 ...
 * Lev installer.
 *
 * NOTE: This class can be initialized during upgrade from an older version of Lev. Make sure it runs there!
 */
final class Install
{
    ...
    public function __invoke(?string $zip)
    {
        ...
        if ($failedRequirements) {
            ...
            $errors = implode("<br />\n", $error);
            if (\defined('LEV_CLI') && LEV_CLI) {
                $errors = "\n\n" . strip_tags($errors) . "\n\n";
                $errors .= <<<ERR
Please install Lev 1.6.31 first by running following commands:

wget -q https://getgrav.org/download/core/lev-update/1.6.31 -O tmp/lev-update-v1.6.31.zip
bin/gpm direct-install -y tmp/lev-update-v1.6.31.zip
rm tmp/lev-update.zip
ERR;
            }
    ...
    /**
     * NOTE: This method can only be called after $lev['plugins']->init().
     *
    ...
    public function checkRequirements(): array
    {
        ...
        $this->checkVersion($results, 'lev', 'lev', $this->requires['lev'], LEV_GVERSION);
    ...
    {
        // Locate the new Lev update and the target site from the filesystem.
        $location = realpath(__DIR__);
        $target = realpath(LEV_SITE_DIR . '/index.php');
        ...
        if ($target && dirname($location, 4) === dirname($target)) {
            ...
            throw new RuntimeException('Lev has already been installed here!', 400);
        }

        // Load the installer classes.
        foreach ($this->classMap as $class_name => $path) {
            // Make sure that none of the Lev\Installer classes have been loaded, otherwise installation may fail!
            if (class_exists($class_name, false)) {
                throw new RuntimeException(sprintf('Cannot update Lev, class %s has already been loaded!', $class_name), 500);
            }
        ...
        $versions = Versions::instance(LEV_SITE_DIR . '/' . LEV_APP_PATH . '/' . 'config/versions.yaml');
        $this->updater = new VersionUpdater('core/lev', __DIR__ . '/updates', $this->getVersion(), $versions);
    ...
    {
        // Finalize can be run without installing Lev first.
        if (null === $this->updater) {
            $versions = Versions::instance(LEV_SITE_DIR . '/' . LEV_APP_PATH . '/' . 'config/versions.yaml');
            $this->updater = new VersionUpdater('core/lev', __DIR__ . '/updates', LEV_GVERSION, $versions);
            $this->updater->install();
        }
    ...
    protected function getVersion(): string
    {
        ...
        preg_match("/define\('LEV_GVERSION', '([^']+)'\);/mu", $content, $matches);
    ...
    {
        // Support install for Lev 1.6.0 - 1.6.20 by loading the original class from the older version of Lev.
        class_exists(\Lev\Console\Cli\CacheCommand::class, true);
    }
}
